<template>
  <div class="hbt-logo" :class="{ collapsed }" @click="handleClick">
    <img 
      v-if="configStore.isShowLogo"
      :src="logoSrc" 
      :alt="logoConfig.alt"
    ...
    />
    <h1 
      v-show="!collapsed && configStore.isShowTitle && (logoConfig.showText !== false)" 
      class="logo-text"
      :class="{ 'animate-title': configStore.isAnimateTitle }"
      :style="textStyle"
    >
      {{ logoConfig.text || 'Lean.Hbt' }}
    </h1>
  </div>
</template>
...
const logoConfig = computed(() => {
  // 如果配置不完整，提供默认配置
  if (!configStore.config.logo || !configStore.config.logo.showText) {
    return {
  ...
  return configStore.config.logo
})
...
// 监听animateTitle配置变化
watch(
  () => configStore.isAnimateTitle,
  (newAnimateTitle) => {
    console.log('[HbtLogo] animateTitle配置变化:', newAnimateTitle)
  }
)
</script>
