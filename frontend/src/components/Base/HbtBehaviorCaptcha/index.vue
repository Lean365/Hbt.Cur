...
const onDragStart = (e: MouseEvent | TouchEvent) => {
  ...
  track.value.push({ x: thumbLeft.value, t: Date.now() })
  ...
  const move = (ev: MouseEvent | TouchEvent) => {
    ...
    track.value.push({ x: left, t: Date.now() })
  }
  ...
  const up = async () => {
    ...
    try {
      ...
      if (thumbLeft.value >= maxLeft.value - 2) {
        // 发送轨迹到后端
        try {
          const { data: { data: token } } = await collectBehavior({
            userId,
            mouseTrack: track.value.map(p => ({ x: Math.round(p.x), y: 0, timestamp: p.t })),
            keyIntervals: [],
            duration: track.value.length > 1 ? track.value[track.value.length - 1].t - track.value[0].t : 0
          })
          
          // 校验行为token
          const { data: { data } } = await validateBehavior(token)
          if (data.success) {
            success.value = true
            emit('success', { token })
          } else {
