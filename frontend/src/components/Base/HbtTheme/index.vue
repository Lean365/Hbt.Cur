...
onMounted(() => {
  ...
  console.log('[HbtTheme] onMounted - 从configStore读取的设置:', currentSettings)
  ...
  if (currentSettings.sidebarColor !== expectedSidebarColor) {
    console.log('[HbtTheme] onMounted - 侧边栏颜色与主题不匹配，使用默认值')
    console.log('[HbtTheme] onMounted - 当前主题:', isDarkMode ? 'dark' : 'light')
    console.log('[HbtTheme] onMounted - 期望颜色:', expectedSidebarColor)
    console.log('[HbtTheme] onMounted - 实际颜色:', currentSettings.sidebarColor)
...
const resetSettings = () => {
  // 重置到当前主题模式的默认配置
  const isDarkMode = themeStore.isDarkMode
  console.log('[HbtTheme] resetSettings - 当前主题模式:', isDarkMode ? 'dark' : 'light')
  const defaultSettings = getDefaultUISettings(isDarkMode) // 根据当前主题模式获取默认配置
  console.log('[HbtTheme] resetSettings - 默认设置:', defaultSettings)
  Object.assign(settings, defaultSettings)
  ...
  configStore.updateConfig('watermark.enabled', defaultAppConfig.watermark.enabled)
  ...
  console.log('[HbtTheme] 设置已重置到默认值（主题模式:', isDarkMode ? 'dark' : 'light', '）')
}
...
const handleThemeModeChange = (e: any) => {
  ...
  console.log('[HbtTheme] handleThemeModeChange - 主题模式切换为:', value)
  ...
  if (value === 'auto') {
    ...
    console.log('[HbtTheme] handleThemeModeChange - 自动模式，侧边栏颜色更新为:', defaultSettings.sidebarColor)
    // 更新配置以匹配主题模式
    ...
    const defaultSettings = getDefaultUISettings(isDarkMode)
    settings.sidebarColor = defaultSettings.sidebarColor
    console.log('[HbtTheme] handleThemeModeChange - 手动模式，侧边栏颜色更新为:', defaultSettings.sidebarColor)
    // 更新配置以匹配主题模式
...
const applySettingsRealtime = () => {
  console.log('[HbtTheme] applySettingsRealtime - 开始应用设置')
  console.log('[HbtTheme] applySettingsRealtime - 当前设置:', settings)
  ...
  console.log('[HbtTheme] applySettingsRealtime - 主题模式:', isDarkMode ? 'dark' : 'light')
  ...
  console.log('[HbtTheme] applySettingsRealtime - 侧边栏颜色已设置:', settings.sidebarColor)
  ...
  console.log('[HbtTheme] applySettingsRealtime - 设置应用完成')
}
...
const handleBreadcrumbChange = (checked: any) => {
  ...
  console.log('[HbtTheme] 面包屑变更:', isChecked)
  
  // 如果启用面包屑，则禁用标签页
  if (isChecked) {
    console.log('[HbtTheme] 启用面包屑，禁用标签页')
    settings.showBreadcrumb = true
    settings.showTabs = false
  } else {
    // 如果禁用面包屑，则启用标签页（确保至少有一个导航方式）
    console.log('[HbtTheme] 禁用面包屑，启用标签页')
...
const handleTabsChange = (checked: any) => {
  const isChecked = Boolean(checked)
  console.log('[HbtTheme] 标签页变更:', isChecked)
  
  // 如果启用标签页，则禁用面包屑
  if (isChecked) {
    console.log('[HbtTheme] 启用标签页，禁用面包屑')
    settings.showTabs = true
    settings.showBreadcrumb = false
  } else {
    // 如果禁用标签页，则启用面包屑（确保至少有一个导航方式）
    console.log('[HbtTheme] 禁用标签页，启用面包屑')
    settings.showTabs = false
    settings.showBreadcrumb = true
  }
  handleSettingChange()
