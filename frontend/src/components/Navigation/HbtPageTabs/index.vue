<template>
  <div class="page-tabs-wrapper">
    <!-- 面包屑导航 - 当启用面包屑时显示 -->
    <div v-if="configStore.isShowBreadcrumb" class="breadcrumb-section">
      <a-breadcrumb>
    ...
    <div v-if="panes.length > 0" class="tabs-section">
      ...
      >
        ...
        >
          <template #tab>
            <span class="tab-item">
              <component :is="pane.icon" v-if="pane.icon && configStore.isShowTabIcon" />
...
const loadTabsFromStorage = () => {
  ...
  if (!configStore.isKeepTabs) {
    console.log('[HbtPageTabs] 标签持久化功能未启用，使用默认标签页')
    return
  }
    ...
    if (savedTabs) {
      ...
      if (Array.isArray(parsedTabs) && parsedTabs.length > 0) {
        ...
        if (allTabs.length > MAX_TABS_COUNT) {
          ...
          console.log(`[HbtPageTabs] 标签页数量超过限制(${MAX_TABS_COUNT})，已自动截取前${MAX_TABS_COUNT}个标签页`)
          // 提示用户有标签页被截取
        ...
        console.log('[HbtPageTabs] 从本地存储加载标签页成功:', panes.value)
      }
    }
  } catch (error) {
    console.error('[HbtPageTabs] 加载标签页失败:', error)
  }
}
...
const saveTabsToStorage = () => {
  ...
  if (!configStore.isKeepTabs) {
    console.log('[HbtPageTabs] 标签持久化功能未启用，不保存标签页')
    return
  }
  ...
  try {
    ...
    console.log('[HbtPageTabs] 标签页保存成功:', tabsToSave)
  } catch (error) {
    console.error('[HbtPageTabs] 保存标签页失败:', error)
  }
}
...
const getPageInfoFromRoute = (path: string): { title: string; icon?: any } => {
  ...
  if (path === route.path) {
    ...
    if (matchedRoute?.meta) {
      ...
      // 从路由meta中获取图标，使用与 HbtBreadcrumb 相同的方式
...
// 监听keepTabs配置变化
watch(
  () => configStore.isKeepTabs,
  (newKeepTabs) => {
    console.log('[HbtPageTabs] keepTabs配置变化:', newKeepTabs)
    ...
    if (newKeepTabs) {
      ...
      try {
        ...
        console.log('[HbtPageTabs] 已清除标签页本地存储')
      } catch (error) {
        console.error('[HbtPageTabs] 清除标签页本地存储失败:', error)
      }
...
watch(
  ...
  (newShowTabIcon) => {
    console.log('[HbtPageTabs] showTabIcon配置变化:', newShowTabIcon)
  }
)

// 监听语言变化，更新所有标签页标题
watch(
  () => t('menu.home'), // 监听首页翻译变化
  () => {
    console.log('[HbtPageTabs] 语言变化，更新标签页标题')
  ...
  (newPath) => {
    ...
    if (!panes.value.find(pane => pane.key === newPath)) {
      ...
      if (panes.value.length >= MAX_TABS_COUNT) {
        ...
        console.log(`[HbtPageTabs] 标签页数量已达上限(${MAX_TABS_COUNT})，阻止添加新标签页`)
