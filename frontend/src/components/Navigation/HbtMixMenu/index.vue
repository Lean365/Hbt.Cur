...
watch(() => themeStore.isDarkMode, (newValue) => {
  console.log('[HbtMixMenu] 主题模式变化:', newValue ? 'dark' : 'light')
}, { immediate: true })

// 监听主色调变化
watch(() => configStore.currentPrimaryColor, (newColor) => {
  console.log('[HbtMixMenu] 主色调变化:', newColor)
}, { immediate: true })
...
// 处理菜单项
const processMenuItem = (menu: HbtMenu, parentPath: string = ''): any => {
  // 确保menu对象存在
  if (!menu) {
    return {
      key: parentPath || '/',
