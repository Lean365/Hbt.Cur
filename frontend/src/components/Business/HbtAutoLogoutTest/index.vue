...
===================================================================
项目名 : Lean.Hbt
文件名 : HbtAutoLogoutTest/index.vue
创建者 : Claude
创建时间: 2024-03-27
版本号 : v1.0.0
描述    : 自动登出功能测试组件
...
使用方式:
- 仅在开发环境显示
- 通过控制台 window.HbtAutoLogout 访问调试函数
- 使用抽屉组件，默认隐藏
===================================================================
-->
...
<template>
  <div>
    ...
    >
      ...
      <div style="margin-top: 16px;">
        <a-alert
          :message="`调试信息: 使用 window.HbtAutoLogout 在控制台访问更多调试函数`"
          type="info"
          show-icon
        />
      </div>
...
import { getAutoLogoutStatus, resetAutoLogoutTimer, checkAndRefreshToken } from '@/utils/autoLogout'
...
const triggerActivity = () => {
  if (typeof window !== 'undefined' && (window as any).HbtAutoLogout) {
    (window as any).HbtAutoLogout.triggerActivity()
    refreshStatus()
  }
}
...
const testAutoRefresh = async () => {
  if (typeof window !== 'undefined' && (window as any).HbtAutoLogout) {
    await (window as any).HbtAutoLogout.testAutoRefresh()
    refreshStatus()
  }
}
