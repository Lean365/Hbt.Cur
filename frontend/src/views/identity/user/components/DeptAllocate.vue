...
import { getUserDepts, allocateUserDepts } from '@/api/identity/user'
...
const fetchDeptTree = async () => {
  try {
    ...
    if (res.data.code === 200) {
      ...
      deptTree.value = convertToTreeData(res.data.data)
...
const fetchUserDepts = async () => {
  try {
    ...
    if (res.data.code === 200) {
      const deptIds = res.data.data.map((item: any) => Number(item.deptId))
...
const handleSubmit = async () => {
  try {
    ...
    const res = await allocateUserDepts(props.userId, formState.deptIds)
