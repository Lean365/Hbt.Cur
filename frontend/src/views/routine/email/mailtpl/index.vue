    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'mailTplCode'">
          <hbt-dict-tag dict-type="email_template_type" :value="record.mailTplCode" />
        </template>
        <!-- 模板状态列 -->
        <template v-if="column.dataIndex === 'status'">
          <hbt-dict-tag dict-type="email_template_status" :value="record.status" />
        </template>
        <!-- 操作列 -->
...
import type { HbtMailTpl, HbtMailTplQuery } from '@/types/routine/mailtpl'
...
// 查询参数
const queryParams = ref<HbtMailTplQuery>({
  pageIndex: 1,
  pageSize: 10,
...
const tableData = ref<HbtMailTpl[]>([])
const total = ref(0)
...
const previewData = ref<HbtMailTpl>({} as HbtMailTpl)
...
// 行点击
const handleRowClick = (record: HbtMailTpl) => {
...
// 编辑
const handleEdit = (record: HbtMailTpl) => {
...
// 删除
const handleDelete = async (record: HbtMailTpl) => {
  try {
...
// 预览
const handlePreview = (record: HbtMailTpl) => {
