...
import type { HbtMailTpl, HbtMailTplCreate, HbtMailTplUpdate } from '@/types/routine/mailtpl'
...
// 表单数据
const formState = ref<Partial<HbtMailTpl>>({
  mailTplId: undefined,
  mailTplName: '',
  mailTplCode: '',
  mailTplSubject: '',
...
watch(() => props.templateId, async (newVal) => {
  if (newVal) {
    try {
      const res: AxiosResponse<HbtApiResponse<HbtMailTpl>> = await getMailTplById(Number(newVal))
...
const handleOk = async () => {
  try {
    ...
    if (formState.value.mailTplId) {
      // 更新操作
      const updateData: HbtMailTplUpdate = {
      ...
      // 创建操作
      const createData: HbtMailTplCreate = {
        mailTplName: values.mailTplName,
        mailTplCode: values.mailTplCode,
        mailTplSubject: values.mailTplSubject,
        mailTplBody: values.mailTplBody,
