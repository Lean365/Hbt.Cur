<template>
  <div class="mail-container">
    ...
    <hbt-table
      ...
      :default-height="594"
      :row-key="(record: HbtMail) => String(record.mailId)"
...
import type { HbtMail, HbtMailQuery } from '@/types/routine/email/mail'
...
// 查询参数
const queryParams = ref<HbtMailQuery>({
  pageIndex: 1,
  pageSize: 10,
  mailTo: '',
  mailSubject: '',
...
// 处理编辑
const handleEdit = (record: HbtMail) => {
...
// 处理删除
const handleDelete = async (record: HbtMail) => {
  try {
...
// 处理发送邮件
const handleSend = (record: HbtMail) => {
  selectedMailId.value = record.mailId
...
// 处理行点击
const handleRowClick = (record: HbtMail) => {
  selectedMailId.value = record.mailId
