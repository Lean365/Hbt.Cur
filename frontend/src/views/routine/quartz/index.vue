<template>
  <div class="quartz-container">
    ...
    <hbt-table
      ...
      :default-height="594"
      :row-key="(record: HbtQuartzJobDto) => String(record.jobId)"
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'jobStatus'">
          <hbt-dict-tag dict-type="quartz_job_status" :value="record.jobStatus" />
        </template>
...
import type { HbtQuartzJobDto, HbtQuartzJobQueryDto } from '@/types/routine/quartz'
...
// 查询参数
const queryParams = ref<HbtQuartzJobQueryDto>({
  pageIndex: 1,
  pageSize: 10,
  jobName: undefined,
  jobGroup: undefined,
...
// 数据列表
const dataList = ref<HbtQuartzJobDto[]>([])
...
const logData = ref<HbtQuartzJobDto>()
...
// 行点击
const handleRowClick = (record: HbtQuartzJobDto) => {
...
// 修改按钮操作
const handleEdit = (record: HbtQuartzJobDto) => {
  selectedJobId.value = Number(record.jobId)
...
// 删除按钮操作
const handleDelete = async (record: HbtQuartzJobDto) => {
  try {
...
// 执行按钮操作
const handleExecute = async (record: HbtQuartzJobDto) => {
  try {
...
// 切换状态按钮操作
const handleToggleStatus = async (record: HbtQuartzJobDto) => {
  try {
...
// 查看日志按钮操作
const handleViewLog = async (record: HbtQuartzJobDto) => {
  try {
