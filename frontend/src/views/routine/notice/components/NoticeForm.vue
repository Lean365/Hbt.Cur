...
import type { HbtNoticeDto, HbtNoticeCreateDto, HbtNoticeUpdateDto } from '@/types/routine/notice'
...
// 表单数据
const formState = ref<Partial<HbtNoticeDto>>({
  noticeId: undefined,
  noticeTitle: '',
  noticeContent: '',
  noticeType: 1,
...
watch(() => props.noticeId, async (newVal) => {
  if (newVal) {
    try {
      const res: AxiosResponse<HbtApiResponse<HbtNoticeDto>> = await getNoticeDetail(newVal)
...
const handleOk = async () => {
  try {
    ...
    if (formState.value.noticeId) {
      const updateData: HbtNoticeUpdateDto = {
      ...
      const createData: HbtNoticeCreateDto = {
        noticeTitle: formState.value.noticeTitle!,
        noticeContent: formState.value.noticeContent!,
        noticeType: formState.value.noticeType!,
        noticeReceiverId: BigInt(formState.value.noticeReceiverId!),
