<template>
  <div class="notice-container">
    ...
    <hbt-table
      ...
      :default-height="594"
      :row-key="(record: HbtNoticeDto) => String(record.noticeId)"
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'noticeType'">
          <hbt-dict-tag dict-type="notice_type" :value="record.noticeType" />
        </template>
        <template v-else-if="column.dataIndex === 'noticeStatus'">
          <hbt-dict-tag dict-type="notice_status" :value="record.noticeStatus" />
        </template>
    ...
    >
      <template v-if="previewData">
        ...
        <div class="notice-info">
          <span>{{ t('notice.field.noticeType') }}: <hbt-dict-tag dict-type="notice_type" :value="previewData.noticeType" /></span>
...
import type { HbtNoticeDto, HbtNoticeQueryDto } from '@/types/routine/notice'
...
// 查询参数
const queryParams = ref<HbtNoticeQueryDto>({
  pageIndex: 1,
  pageSize: 10,
...
// 数据列表
const dataList = ref<HbtNoticeDto[]>([])
...
const previewData = ref<HbtNoticeDto>()
...
// 行点击
const handleRowClick = (record: HbtNoticeDto) => {
...
// 修改按钮操作
const handleEdit = (record: HbtNoticeDto) => {
  selectedNoticeId.value = Number(record.noticeId)
...
// 删除按钮操作
const handleDelete = async (record: HbtNoticeDto) => {
  try {
...
// 预览按钮操作
const handlePreview = async (record: HbtNoticeDto) => {
  try {
