    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'participationType'">
          <hbt-dict-tag dict-type="topic_participation_type" :value="record.participationType" />
        </template>

        <!-- 参与状态列 -->
        <template v-if="column.dataIndex === 'participationStatus'">
          <hbt-dict-tag dict-type="topic_participation_status" :value="record.participationStatus" />
        </template>
...
import type { HbtNewsTopicParticipant, HbtNewsTopicParticipantQuery } from '@/types/routine/news/topic-participant'
...
const tableData = ref<HbtNewsTopicParticipant[]>([])
const total = ref(0)
...
// 查询参数
const queryParams = ref<HbtNewsTopicParticipantQuery>({
  pageIndex: 1,
  pageSize: 10
})
...
// 查询
const handleQuery = (params: HbtNewsTopicParticipantQuery) => {
...
// 行点击
const handleRowClick = (record: HbtNewsTopicParticipant) => {
...
// 编辑
const handleEdit = (record: HbtNewsTopicParticipant) => {
...
// 删除
const handleDelete = async (record: HbtNewsTopicParticipant) => {
  try {
