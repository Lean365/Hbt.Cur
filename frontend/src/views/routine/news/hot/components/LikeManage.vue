...
import type { HbtNewsLike } from '@/types/routine/news/like'
...
const likeList = ref<HbtNewsLike[]>([])
...
const fetchLikes = async () => {
  try {
    ...
    if (res.data.code === 200) {
      likeList.value = res.data.data.rows
      pagination.total = res.data.data.total
      
      // 计算统计数据
      calculateStats()
    } else {
      message.error(res.data.msg || '获取点赞列表失败')
    }
  } catch (error) {
...
// 移除点赞
const handleRemoveLike = async (record: HbtNewsLike) => {
  try {
    ...
    if (res.data.code === 200) {
      ...
      message.error(res.data.msg || '移除点赞失败')
    }
  } catch (error) {
