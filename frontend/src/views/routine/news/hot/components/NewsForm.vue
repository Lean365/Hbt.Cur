...
import type { HbtNewsCreate, HbtNewsUpdate } from '@/types/routine/news/news'
...
// 表单数据
const formData = reactive<HbtNewsCreate & { newsId?: number }>({
  newsTitle: '',
  newsSubtitle: '',
  newsContent: '',
  newsSummary: '',
...
const handleSubmit = async () => {
  try {
    ...
    if (props.newsId) {
      // 更新
      res = await updateNews(submitData as HbtNewsUpdate)
    } else {
      // 新增
      res = await createNews(submitData as HbtNewsCreate)
    }
