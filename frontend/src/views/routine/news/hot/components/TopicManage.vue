...
import type { HbtNewsTopic } from '@/types/routine/news/topic'
import type { HbtNewsTopicRelation } from '@/types/routine/news/topic-relation'
import { 
  getTopicsByNewsId, 
  relateNewsToTopic,
...
const topicList = ref<HbtNewsTopicRelation[]>([])
const availableTopics = ref<HbtNewsTopic[]>([])
...
// 移除话题关联
const handleRemoveTopic = async (record: HbtNewsTopic) => {
  try {
...
// 查看话题
const handleViewTopic = (record: HbtNewsTopic) => {
  // 实现查看话题逻辑
