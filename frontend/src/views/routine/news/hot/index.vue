<template>
  <div class="news-container">
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'status'">
          <hbt-dict-tag dict-type="news_status" :value="record.status" />
        </template>
    ...
    <!-- 新闻表单对话框 -->
    <news-form
      v-model:visible="newsFormVisible"
      :title="formTitle"
...
import type { HbtNews, HbtNewsQuery } from '@/types/routine/news/news'
...
// 查询参数
const queryParams = ref<HbtNewsQuery>({
  pageIndex: 1,
  pageSize: 10,
  newsTitle: '',
  newsContent: '',
...
// 行点击
const handleRowClick = (record: HbtNews) => {
...
// 编辑
const handleEdit = (record: HbtNews) => {
...
// 查看
const handleView = (record: HbtNews) => {
...
// 删除
const handleDelete = async (record: HbtNews) => {
  try {
...
// 查看评论
const handleViewComments = (record: HbtNews) => {
  selectedNewsId.value = record.newsId
  commentDialogVisible.value = true
}

// 查看点赞
const handleViewLikes = (record: HbtNews) => {
  selectedNewsId.value = record.newsId
  likeDialogVisible.value = true
}

// 管理话题
const handleManageTopics = (record: HbtNews) => {
  selectedNewsId.value = record.newsId
