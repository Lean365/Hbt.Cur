<template>
  <div class="mail-container">
    ...
    <hbt-table
      ...
      :default-height="594"
      :row-key="(record: HbtMailDto) => String(record.mailId)"
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'mailStatus'">
          <hbt-dict-tag dict-type="sys_yes_no" :value="record.mailStatus" />
        </template>
...
import type { HbtMailDto, HbtMailQueryDto } from '@/types/routine/mail'
...
// 查询参数
const queryParams = ref<HbtMailQueryDto>({
  pageIndex: 1,
  pageSize: 10,
  mailTo: '',
  mailSubject: '',
...
const total = ref(0)
const tableData = ref<HbtMailDto[]>([])
...
// 处理编辑
const handleEdit = (record: HbtMailDto) => {
...
// 处理发送邮件
const handleSend = (record: HbtMailDto) => {
  selectedMailId.value = record.mailId
...
// 处理行点击
const handleRowClick = (record: HbtMailDto) => {
  selectedMailId.value = record.mailId
