    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'priority'">
          <hbt-dict-tag dict-type="schedule_priority" :value="record.priority" />
        </template>
        <template v-if="column.dataIndex === 'status'">
          <hbt-dict-tag dict-type="schedule_status" :value="record.status" />
        </template>
      ...
      <div class="view-content">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item :label="t('schedule.priority')">
            <hbt-dict-tag dict-type="schedule_priority" :value="viewData.priority" />
          </a-descriptions-item>
          <a-descriptions-item :label="t('schedule.status')">
            <hbt-dict-tag dict-type="schedule_status" :value="viewData.status" />
          </a-descriptions-item>
        </a-descriptions>
      </div>
    </a-modal>
...
import type { HbtSchedule, HbtScheduleQuery } from '@/types/routine/schedule'
...
// 查询参数
const queryParams = ref<HbtScheduleQuery>({
  pageIndex: 1,
  pageSize: 10,
  title: undefined,
  status: undefined,
...
// 表格数据
const tableData = ref<HbtSchedule[]>([])
...
const viewData = ref<HbtSchedule>({} as HbtSchedule)
...
// 行点击
const handleRowClick = (record: HbtSchedule) => {
...
// 编辑
const handleEdit = (record: HbtSchedule) => {
...
// 查看
const handleView = (record: HbtSchedule) => {
  viewData.value = record
  viewVisible.value = true
}

// 删除
const handleDelete = async (record: HbtSchedule) => {
  try {
