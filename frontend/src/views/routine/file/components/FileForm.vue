...
import { getFileDetail, createFile, updateFile, uploadFile } from '@/api/routine/file'
import type { HbtFileDto, HbtFileCreateDto, HbtFileUpdateDto, HbtFileUploadDto } from '@/types/routine/file'
...
const formState = reactive<HbtFileUploadDto>({
  fileOriginalName: '',
  fileExtension: '',
  fileName: 'random',
...
const handleOk = async () => {
  try {
    ...
    if (response.data && response.data.code === 200) {
      ...
      message.error(response.data?.msg || t('routine.file.upload.failed'))
    }
  } catch (error: any) {
