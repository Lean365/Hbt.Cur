  ...
  >
    <a-descriptions :column="2" bordered>
      ...
      <a-descriptions-item :label="t('routine.file.status')" :span="1">
        <hbt-dict-tag dict-type="sys_normal_disable" :value="form.fileStatus" />
      </a-descriptions-item>
...
import type { HbtFileDto } from '@/types/routine/file'
import { getFileDetail, downloadFile } from '@/api/routine/file'
...
// 表单数据
const form = reactive<HbtFileDto & { fileUploaderName?: string }>(Object.assign({
  id: 0,
  createBy: '',
  isDeleted: 0,
  fileId: 0,
...
const getInfo = async (fileId: number) => {
  try {
    ...
    if (res.data.code === 200) {
      ...
      message.error(res.data.msg || t('routine.file.downloadFailed'))
    }
  } catch (error) {
    console.error(error)
