<template>
  <div class="file-container">
    ...
    <hbt-table
      ...
      :row-key="(record: HbtFileDto) => String(record.fileId)"
...
import { getFileList, deleteFile, downloadFile, uploadFile, exportFileList, batchDeleteFile } from '@/api/routine/file'
import type { HbtFileDto, HbtFileQueryDto } from '@/types/routine/file'
...
// === 查询参数 ===
const queryParams = ref<HbtFileQueryDto>({
  pageIndex: 1,
  pageSize: 10,
  fileName: '',
  fileType: '',
...
// 行点击
const handleRowClick = (record: HbtFileDto) => {
...
// 删除文件
const handleDelete = async (record: HbtFileDto) => {
  try {
...
// 删除确认
const handleDeleteConfirm = (record: HbtFileDto) => {
  Modal.confirm({
...
// 编辑
const handleEdit = (record: HbtFileDto) => {
  selectedFileId.value = record.fileId
...
// 查看
const handleView = (record: HbtFileDto) => {
  selectedFileId.value = record.fileId
