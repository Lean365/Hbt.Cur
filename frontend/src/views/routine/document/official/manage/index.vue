<template>
  <div class="file-container">
    ...
    <hbt-table
      ...
      :row-key="(record: HbtFile) => String(record.fileId)"
...
import type { HbtFile, HbtFileQuery } from '@/types/routine/document/file'
...
// === 查询参数 ===
const queryParams = ref<HbtFileQuery>({
  pageIndex: 1,
  pageSize: 10,
  fileName: '',
  fileType: '',
...
// 行点击
const handleRowClick = (record: HbtFile) => {
...
// 删除文件
const handleDelete = async (record: HbtFile) => {
  try {
...
// 删除确认
const handleDeleteConfirm = (record: HbtFile) => {
  Modal.confirm({
...
// 编辑
const handleEdit = (record: HbtFile) => {
  selectedFileId.value = record.fileId
...
// 查看
const handleView = (record: HbtFile) => {
  selectedFileId.value = record.fileId
