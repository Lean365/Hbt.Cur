    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'status'">
          <hbt-dict-tag dict-type="driver_status" :value="record.status" />
        </template>

        <!-- 驾驶证类型列 -->
        <template v-if="column.dataIndex === 'licenseType'">
          <hbt-dict-tag dict-type="driver_license_type" :value="record.licenseType" />
        </template>

        <!-- 驾驶证状态列 -->
        <template v-if="column.dataIndex === 'licenseStatus'">
          <hbt-dict-tag dict-type="driver_license_status" :value="record.licenseStatus" />
        </template>
...
import type { HbtDriver, HbtDriverQuery } from '@/types/routine/vehicle/driver'
...
// 查询参数
const queryParams = ref<HbtDriverQuery>({
  pageIndex: 1,
  pageSize: 10
})
...
// 查询
const handleQuery = (params: HbtDriverQuery) => {
...
// 行点击
const handleRowClick = (record: HbtDriver) => {
...
// 编辑
const handleEdit = (record: HbtDriver) => {
...
// 删除
const handleDelete = async (record: HbtDriver) => {
  try {
