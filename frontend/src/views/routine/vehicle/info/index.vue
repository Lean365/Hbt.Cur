    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'vehicleType'">
          <hbt-dict-tag dict-type="vehicle_type" :value="record.vehicleType" />
        </template>

        <!-- 车辆状态列 -->
        <template v-if="column.dataIndex === 'status'">
          <hbt-dict-tag dict-type="vehicle_status" :value="record.status" />
        </template>
...
import type { HbtVehicle, HbtVehicleQuery } from '@/types/routine/vehicle/vehicle'
...
const tableData = ref<HbtVehicle[]>([])
const total = ref(0)
...
// 查询参数
const queryParams = ref<HbtVehicleQuery>({
  pageIndex: 1,
  pageSize: 10
})
...
// 查询
const handleQuery = (params: HbtVehicleQuery) => {
...
// 行点击
const handleRowClick = (record: HbtVehicle) => {
...
// 编辑
const handleEdit = (record: HbtVehicle) => {
...
// 删除
const handleDelete = async (record: HbtVehicle) => {
  try {
