    ...
    <div class="step-content">
      ...
      <div v-show="currentStep === 0" class="step-panel">
        ...
        >
          ...
          <a-form-item label="流程分类" name="schemeCategory">
            <hbt-select v-model:value="formState.schemeCategory" dict-type="workflow_category" type="select" 
            :show-all="false" :placeholder="t('generator.config.placeholder.workflowCategory')" style="width: 100%" />
          </a-form-item>
          <a-form-item label="版本" name="version">
            <hbt-select v-model:value="formState.version" dict-type="workflow_version" type="select" 
            :show-all="false" :placeholder="t('generator.config.placeholder.workflowVersion')" style="width: 100%" />
          </a-form-item>
          <a-form-item label="状态" name="status">
            <hbt-select v-model:value="formState.status" dict-type="workflow_definition_status" type="select" 
      ...
      <div v-show="currentStep === 2" class="step-panel">
        <div class="flow-canvas-wrapper">
          <hbt-flow v-model:value="formState.schemeConfig" :width="1600" :height="800" />
        </div>
      </div>
    </div>
...
import type { HbtScheme, HbtSchemeCreate, HbtSchemeUpdate } from '@/types/workflow/scheme'
...
const formState = reactive<HbtSchemeCreate & { status: number }>({
  schemeKey: '',
  schemeName: '',
  schemeCategory: 1,
  version: '1.0',
...
const handleSubmit = async () => {
  try {
    loading.value = true
    console.log('提交前的 schemeConfig:', formState.schemeConfig)
    ...
    console.log('转换后的 schemeConfig:', schemeConfigStr)
    
    const data: HbtSchemeCreate = {
    ...
    if (props.definitionId) {
      const updateData: HbtSchemeUpdate = {
        ...data,
        schemeId: props.definitionId
      }
