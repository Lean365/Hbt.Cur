  ...
  <div class="password-recovery-container">
    ...
    <div v-if="currentStep === 0" class="step-content">
      <a-form
        ...
        layout="vertical"
      >
        <a-form-item name="captcha" :label="t('identity.auth.passwordRecovery.captcha')" v-if="showCaptcha">
          <!-- 滑块验证码 -->
          <HbtSliderCaptcha
    ...
    <div v-if="currentStep === 1" class="step-content">
      <a-form
        ...
        layout="vertical"
      >
        <a-form-item name="userName" :label="t('identity.auth.passwordRecovery.userName')">
          <a-input
            v-model:value="identityForm.userName"
            :placeholder="t('identity.auth.passwordRecovery.userNamePlaceholder')"
            size="large"
          >
            <template #prefix>
              <user-outlined />
        ...
        <a-form-item name="email" :label="t('identity.auth.passwordRecovery.email')">
          <a-input
            v-model:value="identityForm.email"
            :placeholder="t('identity.auth.passwordRecovery.emailPlaceholder')"
            size="large"
            type="email"
          >
            <template #prefix>
    ...
    <div v-if="currentStep === 2" class="step-content">
      ...
      <a-form
        ...
        layout="vertical"
      >
        <a-form-item name="verificationCode" :label="t('identity.auth.passwordRecovery.verificationCode')">
          <a-input
            v-model:value="verificationForm.verificationCode"
            :placeholder="t('identity.auth.passwordRecovery.verificationCodePlaceholder')"
            size="large"
            :maxlength="6"
          >
            <template #prefix>
    ...
    <div v-if="currentStep === 3" class="step-content">
      <a-form
        ...
        layout="vertical"
      >
        <a-form-item name="newPassword" :label="t('identity.auth.passwordRecovery.newPassword')">
          <a-input-password
            v-model:value="passwordForm.newPassword"
            :placeholder="t('identity.auth.passwordRecovery.newPasswordPlaceholder')"
            size="large"
          >
            <template #prefix>
              <lock-outlined />
        ...
        <a-form-item name="confirmPassword" :label="t('identity.auth.passwordRecovery.confirmPassword')">
          <a-input-password
            v-model:value="passwordForm.confirmPassword"
            :placeholder="t('identity.auth.passwordRecovery.confirmPasswordPlaceholder')"
            size="large"
          >
            <template #prefix>
              <lock-outlined />
...
import { 
  ...
  verifyEmailCode, 
  resetPassword 
} from '@/api/identity/auth/userRecovery'
...
const handleIdentityVerification = async () => {
  try {
    ...
    if (data.data.success) {
      ...
      message.error(data.data.message || t('identity.auth.passwordRecovery.error.identityVerificationFailed'))
    }
...
const sendVerificationCode = async () => {
  try {
    ...
    if (data.data.success) {
      ...
      message.error(data.data.message || t('identity.auth.passwordRecovery.error.sendCodeFailed'))
    }
  } catch (error: any) {
...
const handleEmailVerification = async () => {
  try {
    ...
    if (data.data.success) {
      verificationToken.value = data.data.resetToken
      ...
      message.error(data.data.message || t('identity.auth.passwordRecovery.error.emailVerificationFailed'))
    }
...
const handlePasswordReset = async () => {
  try {
    ...
    if (data.data.success) {
      ...
      message.error(data.data.message || t('identity.auth.passwordRecovery.error.passwordResetFailed'))
    }
