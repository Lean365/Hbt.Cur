...
const currentTable = ref<HbtGenTable | null>(null)
...
/** 表格选择框事件 */
const handleSelectionChange = (selectedRowKeys: (string | number)[], selection: HbtGenTable[]) => {
  selectedTables.value = selection
}

/** 查看按钮点击事件 */
const handleView = async (record: HbtGenTable) => {
...
/** 单表导入按钮点击事件 */
const handleSingleImport = async (record: HbtGenTable) => {
  if (!formState.databaseName) {
    message.warning('请先选择数据库')
    return
  }
...
/** 导出按钮点击事件 */
const handleExport = (record: HbtGenTable) => {
...
/** 删除按钮点击事件 */
const handleDelete = (record: HbtGenTable) => {
