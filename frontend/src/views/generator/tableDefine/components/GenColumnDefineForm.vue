...
import type { HbtGenColumnDefine, HbtGenColumnDefineCreate, HbtGenColumnDefineUpdate } from '@/types/generator/genColumnDefine'
...
// 表单数据
const formData = reactive<HbtGenColumnDefineCreate>({
...
const getDetail = async () => {
  ...
  try {
    ...
    if (res.data?.data?.items?.length > 0) {
      Object.assign(formData, res.data.data.items[0])
    }
  } finally {
    loading.value = false
  }
