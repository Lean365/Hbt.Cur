...
import type { HbtGenColumnDefine, HbtGenColumnDefineCreate, HbtGenColumnDefineUpdate, HbtGenColumnDefineQuery } from '@/types/generator/genColumnDefine'
...
const dataSource = ref<HbtGenColumnDefine[]>([])
const selectedRowKeys = ref<(string | number)[]>([])
const showSearch = ref(true)
const queryParams = ref<HbtGenColumnDefineQuery>({
  pageIndex: 1,
  pageSize: 10,
...
const fetchData = async () => {
  if (!props.tableId || props.tableId <= 0) {
  ...
  try {
    ...
    if (res.data.code === 200) {
      // 添加空值检查
      if (res.data.data) {
        dataSource.value = (res.data.data.items || []).map(item => ({
          ...item,
          isEditing: false // 设置初始状态为非编辑
        }))
        pagination.total = res.data.data.totalNum || 0
      } else {
        dataSource.value = []
        pagination.total = 0
        console.log('列定义数据为空')
...
const handleAdd = async () => {
  try {
    const newColumn: HbtGenColumnDefine = {
      genColumnDefineId: 0,
      tableId: props.tableId,
      columnName: '',
      columnComment: '',
...
// 编辑按钮操作
const handleEdit = (record: HbtGenColumnDefine) => {
  record.isEditing = true // 设置为编辑状态
}

// 删除按钮操作
const handleDelete = async (record: HbtGenColumnDefine) => {
  try {
    if (record.genColumnDefineId === 0) {
      // 如果是新增的记录，直接从数据源中移除
...
const handleBatchDelete = async () => {
  ...
  try {
    await batchDeleteColumnDefine(selectedRowKeys.value.map(id => Number(id)))
...
// 处理选择框变化
const handleSelectChange = (record: HbtGenColumnDefine, field: string, value: string) => {
...
// 处理开关变化
const handleSwitchChange = (record: HbtGenColumnDefine, field: string, value: number) => {
  (record as any)[field] = value
  validateField(record, field)
}

// 处理数字输入框变化
const handleNumberChange = (record: HbtGenColumnDefine, field: string, value: number) => {
  (record as any)[field] = value
  validateField(record, field)
}

// 处理文本输入框变化
const handleInputChange = (record: HbtGenColumnDefine, field: string, value: string) => {
  if (field === 'columnName') {
    // 转换为帕斯卡命名法
...
// 验证单个字段
const validateField = (record: HbtGenColumnDefine, field: string) => {
...
// 处理保存按钮操作
const handleSave = async (record: HbtGenColumnDefine) => {
  try {
    // 使用每行的formRef进行校验
