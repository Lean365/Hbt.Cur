...
export function setIconRandomColor(element: HTMLElement, seed?: string): void {
  ...
  // 添加随机颜色类
  element.classList.add('hbt-icon-random');
...
export function setIconThemeColor(element: HTMLElement, theme: 'primary' | 'success' | 'warning' | 'error' | 'info'): void {
  // 移除随机颜色类
  element.classList.remove('hbt-icon-random');
  
  // 添加主题颜色类
  element.classList.add(`hbt-icon-${theme}`);
...
export function setIconGradientColor(element: HTMLElement, gradientType: 'primary' | 'success' | 'warning' | 'error'): void {
  // 移除其他颜色类
  element.classList.remove('hbt-icon-random', 'hbt-icon-primary', 'hbt-icon-success', 'hbt-icon-warning', 'hbt-icon-error', 'hbt-icon-info');
  
  // 添加渐变颜色类
  element.classList.add(`hbt-icon-gradient-${gradientType}`);
...
export function setIconAnimation(element: HTMLElement, animation: 'pulse' | 'spin' | 'bounce' | 'shake'): void {
  // 移除其他动画类
  element.classList.remove('hbt-icon-pulse', 'hbt-icon-spin', 'hbt-icon-bounce', 'hbt-icon-shake');
  
  // 添加动画类
  element.classList.add(`hbt-icon-${animation}`);
...
export function setIconSize(element: HTMLElement, size: 'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl'): void {
  // 移除其他尺寸类
  element.classList.remove('hbt-icon-xs', 'hbt-icon-sm', 'hbt-icon-md', 'hbt-icon-lg', 'hbt-icon-xl', 'hbt-icon-xxl');
  
  // 添加尺寸类
  element.classList.add(`hbt-icon-${size}`);
...
export function setIconStatus(element: HTMLElement, status: 'disabled' | 'loading' | 'selected' | 'highlight'): void {
  // 移除其他状态类
  element.classList.remove('hbt-icon-disabled', 'hbt-icon-loading', 'hbt-icon-selected', 'hbt-icon-highlight');
  
  // 添加状态类
  element.classList.add(`hbt-icon-${status}`);
...
export function clearIconStyles(element: HTMLElement): void {
  ...
  const classesToRemove = [
    'hbt-icon-random', 'hbt-icon-primary', 'hbt-icon-success', 'hbt-icon-warning', 'hbt-icon-error', 'hbt-icon-info',
    'hbt-icon-gradient-primary', 'hbt-icon-gradient-success', 'hbt-icon-gradient-warning', 'hbt-icon-gradient-error',
    'hbt-icon-pulse', 'hbt-icon-spin', 'hbt-icon-bounce', 'hbt-icon-shake',
    'hbt-icon-xs', 'hbt-icon-sm', 'hbt-icon-md', 'hbt-icon-lg', 'hbt-icon-xl', 'hbt-icon-xxl',
    'hbt-icon-disabled', 'hbt-icon-loading', 'hbt-icon-selected', 'hbt-icon-highlight'
  ];
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node instanceof HTMLElement) {
          // 检查新添加的元素是否包含图标
          const icons = node.querySelectorAll('.anticon, .hbt-icon-random');
          icons.forEach((icon) => {
