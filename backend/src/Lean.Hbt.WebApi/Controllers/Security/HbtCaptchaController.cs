...
public class HbtCaptchaController : HbtBaseController
{
    ...
    private readonly HbtCaptchaOptions _captchaOptions;
    ...
    public HbtCaptchaController(
        ...
        IOptions<HbtCaptchaOptions> captchaOptions,
        IHbtLogger logger,
    ...
    [HttpGet("config")]
    [AllowAnonymous]
    public ActionResult<HbtApiResult<CaptchaConfigDto>> GetCaptchaConfig([FromQuery] string? scenario = null)
    {
        ...
        {
            _logger.Warn("配置的验证码类型 '{Type}' 无效，默认使用 'Slider'", _captchaOptions.Type);
            validType = "Slider";
        }
        ...
        return Ok(HbtApiResult<CaptchaConfigDto>.Success(config));
    }
    ...
    [HttpGet("slider")]
    [AllowAnonymous]
    public async Task<ActionResult<HbtApiResult<SliderCaptchaDto>>> GenerateSliderAsync()
    {
        ...
        try
        {
            ...
            return Ok(HbtApiResult<SliderCaptchaDto>.Success(result));
        }
        catch (Exception ex)
        {
    ...
    [AllowAnonymous]
    public async Task<ActionResult<HbtApiResult<CaptchaResultDto>>> ValidateSliderAsync([FromBody] SliderValidateDto request)
    {
        ...
        try
        {
            ...
            return Ok(HbtApiResult<CaptchaResultDto>.Success(result));
        }
        catch (Exception ex)
        {
    ...
    [AllowAnonymous]
    public async Task<ActionResult<HbtApiResult<string>>> CollectBehaviorDataAsync([FromBody] BehaviorDataDto request)
    {
        ...
        return Ok(HbtApiResult<string>.Success(token));
    }
    ...
    [AllowAnonymous]
    public async Task<ActionResult<HbtApiResult<CaptchaResultDto>>> ValidateBehaviorAsync([FromBody] BehaviorValidateDto request)
    {
