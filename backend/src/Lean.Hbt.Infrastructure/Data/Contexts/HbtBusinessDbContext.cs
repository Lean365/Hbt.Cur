...
namespace Lean.Hbt.Infrastructure.Data.Contexts
{
    ...
    /// </summary>
    public class HbtBusinessDbContext : IHbtBusinessDbContext
    {
        /// <summary>
        /// SqlSugar数据库客户端实例
        /// </summary>
        ...
        public HbtBusinessDbContext(string connectionString, IOptions<HbtDbOptions> dbOptions, IHbtLogger logger, IHbtCurrentUser currentUser, IServiceProvider serviceProvider, IConfiguration configuration)
        {
            _logger = logger;
        ...
        private async Task InitializeBusinessDatabaseAsync()
        {
            try
            {
                ...
                // 2.获取所有业务实体类型（除了Audit、Identity、SignalR、Workflow、Generator）
                var entityTypes = GetBusinessEntityTypes();

                // 3.创建/更新表
                await CreateOrUpdateTablesAsync(entityTypes, "业务数据库");
        ...
        /// <summary>
        /// 获取所有业务实体类型（除了Audit、Identity、SignalR、Workflow、Generator）
        /// </summary>
        private Type[] GetBusinessEntityTypes()
        {
            return AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(t => t.IsClass && !t.IsAbstract && t.IsPublic &&
                           ...
                           !t.Namespace.StartsWith("Lean.Hbt.Domain.Entities.Workflow") &&
                           !t.Namespace.StartsWith("Lean.Hbt.Domain.Entities.Generator") &&
...
/// 业务数据库上下文接口
/// </summary>
public interface IHbtBusinessDbContext
{
    /// <summary>
    /// 获取数据库客户端实例
    /// </summary>
