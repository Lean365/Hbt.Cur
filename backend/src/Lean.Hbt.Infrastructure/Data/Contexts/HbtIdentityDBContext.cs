...
namespace Lean.Hbt.Infrastructure.Data.Contexts
{
    /// <summary>
    /// 认证数据库上下文类
    /// 负责管理认证数据库连接、事务、实体映射和数据库操作
    /// </summary>
    public class HbtIdentityDBContext : IHbtIdentityDBContext
    {
        /// <summary>
        /// SqlSugar数据库客户端实例
        /// </summary>
        ...
        public HbtIdentityDBContext(string connectionString, IOptions<HbtDbOptions> dbOptions, IHbtLogger logger, IHbtCurrentUser currentUser, IServiceProvider serviceProvider, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public async Task InitializeAsync()
        {
            try
            {
                _logger.Info("开始初始化认证数据库");

                // 初始化认证数据库
                await InitializeAuthDatabaseAsync();

                _logger.Info("认证数据库初始化完成");
            }
            catch (Exception ex)
            {
                _logger.Error("初始化认证数据库失败", ex);
                throw;
            }
        }
        ...
        /// 初始化认证数据库
        /// </summary>
        private async Task InitializeAuthDatabaseAsync()
        {
            try
            {
                _logger.Info("开始初始化认证数据库");
                ...
                _logger.Info("认证数据库检查/创建成功");

                // 2.获取所有认证相关实体类型（Audit、Identity、SignalR）
                var entityTypes = GetAuthEntityTypes();

                // 3.创建/更新表
                await CreateOrUpdateTablesAsync(entityTypes, "认证数据库");
        ...
        /// <summary>
        /// 获取所有认证相关实体类型（Audit、Identity、SignalR）
        /// </summary>
        private Type[] GetAuthEntityTypes()
        {
            return AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(t => t.IsClass && !t.IsAbstract && t.IsPublic &&
                           t.Namespace != null &&
                           (t.Namespace.StartsWith("Lean.Hbt.Domain.Entities.Audit") ||
                            t.Namespace.StartsWith("Lean.Hbt.Domain.Entities.Identity") ||
                            t.Namespace.StartsWith("Lean.Hbt.Domain.Entities.SignalR")) &&
...
/// 认证数据库上下文接口
/// </summary>
public interface IHbtIdentityDBContext
{
    /// <summary>
    /// 获取数据库客户端实例
    /// </summary>
