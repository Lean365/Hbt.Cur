...
namespace Lean.Hbt.Infrastructure.Data.Contexts
{
    /// <summary>
    /// 代码生成数据库上下文类
    /// 负责管理代码生成数据库连接、事务、实体映射和数据库操作
    /// </summary>
    public class HbtGeneratorDbContext : IHbtGeneratorDbContext
    {
        /// <summary>
        /// SqlSugar数据库客户端实例
        /// </summary>
        ...
        public HbtGeneratorDbContext(string connectionString, IOptions<HbtDbOptions> dbOptions, IHbtLogger logger, IHbtCurrentUser currentUser, IServiceProvider serviceProvider, IConfiguration configuration)
        {
            _logger = logger;
        ...
        private async Task InitializeGeneratorDatabaseAsync()
        {
            try
            {
                ...
                _logger.Info("代码生成数据库检查/创建成功");

                // 2.获取所有Generator实体类型
                var entityTypes = GetGeneratorEntityTypes();

                // 3.创建/更新表
                await CreateOrUpdateTablesAsync(entityTypes, "代码生成数据库");
...
/// 代码生成数据库上下文接口
/// </summary>
public interface IHbtGeneratorDbContext
{
    /// <summary>
    /// 获取数据库客户端实例
    /// </summary>
