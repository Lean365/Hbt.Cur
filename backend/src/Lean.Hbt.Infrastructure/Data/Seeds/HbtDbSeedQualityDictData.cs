...
public class HbtDbSeedQualityDictData
{
    ...
    public HbtDbSeedQualityDictData(IHbtRepository<HbtDictData> dictDataRepository, IHbtLogger logger)
    {
    ...
    public async Task<(int, int)> InitializeQualityDictDataAsync(long tenantId)
    {
        ...
        foreach (var dictData in defaultDictData)
        {
            ...
            if (existingDictData == null)
            {
                ...
                insertCount++;
                _logger.Info($"[创建] 质量字典数据 '{dictData.DictLabel}' 创建成功");
            }
            else
            {
                ...
                updateCount++;
                _logger.Info($"[更新] 质量字典数据 '{existingDictData.DictLabel}' 更新成功");
            }
        }
