...
/// 关联关系数据初始化类
/// </summary>
public class HbtDbSeedIdentityRelation
{
    ...
    /// 构造函数
    /// </summary>
    public HbtDbSeedIdentityRelation(
    ...
    public async Task<(int, int)> InitializeRelationsAsync()
    {
        ...
        // 获取第一个可用的租户作为默认租户
        var defaultTenant = await TenantRepository.GetFirstAsync(t => t.IsDeleted == 0);
        if (defaultTenant == null)
        {
            _logger.Error("未找到可用租户，无法初始化用户租户关联关系");
            return (0, 0);
        }

        // 初始化用户租户关联关系
        var userTenantResult = await InitializeUserTenantRelationsAsync(defaultTenant.ConfigId);
        var userTenantInsert = userTenantResult.Item1;
        var userTenantUpdate = userTenantResult.Item2;
        ...
        foreach (var role in allRoles)
        {
            ...
            foreach (var dept in depts)
            {
                ...
                else
                {
                    ...
                    existingRoleDept.UpdateBy = "Hbt365";
            ...
            try
            {
                ...
                foreach (var menu in allMenus)
                {
                    if (existingRelationDict.TryGetValue(menu.Id, out var existingRelation))
                    {
                        ...
                        existingRelation.UpdateBy = "Hbt365";
    ...
    /// 初始化用户租户关联数据
    /// </summary>
    private async Task<(int, int)> InitializeUserTenantRelationsAsync(string configId)
    {
        int insertCount = 0;
        int updateCount = 0;

        // 获取所有用户和租户
        var users = await UserRepository.GetListAsync(u => u.IsDeleted == 0);
        var allTenants = await TenantRepository.GetListAsync(t => t.IsDeleted == 0);
        var existingRelations = await UserTenantRepository.GetListAsync(ut => true);
        ...
        foreach (var user in users)
        {
            if (user.UserName == "admin")
            {
                ...
                foreach (var tenant in allTenants)
                {
                    var existingRelation = existingRelations.FirstOrDefault(ut =>
                        ut.UserId == user.Id && ut.ConfigId == tenant.ConfigId);
                    ...
                    if (existingRelation == null)
                    {
                        ...
                        await UserTenantRepository.CreateAsync(userTenantRelation);
                        insertCount++;
                    }
                    else
                    {
                        ...
                        existingRelation.DeleteTime = null;
                        existingRelation.UpdateTime = DateTime.Now;
                        await UserTenantRepository.UpdateAsync(existingRelation);
                        updateCount++;
                    }
                }
            }
            else
            {
                // 其他用户只关联到指定租户
                var existingRelation = existingRelations.FirstOrDefault(ut =>
                    ut.UserId == user.Id && ut.ConfigId == configId);
