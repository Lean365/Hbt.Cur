...
namespace Lean.Hbt.Infrastructure.SignalR.Cache
{
    /// <summary>
    /// SignalR内存缓存实现
    /// </summary>
    ...
    /// 包含内存溢出保护机制
    /// </remarks>
    public class HbtSignalRMemoryCache : IHbtSignalRCacheService, IDisposable
    {
        /// <summary>
        /// 内存缓存实例
        /// </summary>
        ...
        /// 缓存配置选项
        /// </summary>
        private readonly HbtSignalRCacheOptions _options;
        ...
        public HbtSignalRMemoryCache(
            IMemoryCache cache,
            IOptions<HbtSignalRCacheOptions> options)
        {
            _cache = cache;
        ...
        public async Task SetAsync(string key, HbtOnlineUser user, TimeSpan? expiry = null)
        {
            // 检查用户连接数限制
            ...
            var options = new MemoryCacheEntryOptions()
                ...
                {
                    // 当缓存项被移除时，更新缓存大小
                    if (_options.EnableSizeLimit && value is HbtOnlineUser u)
                    {
        ...
        public async Task<HbtOnlineUser?> GetAsync(string key)
        {
            _cache.TryGetValue<HbtOnlineUser>(key, out var user);
            if (user != null)
            {
                // 更新最后访问时间
        ...
        {
            if (_options.EnableSizeLimit && _cache.TryGetValue<HbtOnlineUser>(key, out var user))
            {
        ...
        private async Task CleanupExpiredConnectionsAsync()
        {
            ...
            foreach (var userConnections in _userConnections.Values)
            {
                foreach (var connectionId in userConnections)
                {
                    ...
                    if (_cache.TryGetValue<HbtOnlineUser>(key, out var user))
                    {
        ...
        /// 估算对象大小
        /// </summary>
        private long EstimateObjectSize(HbtOnlineUser user)
        {
            // 粗略估算每个用户对象占用的内存大小
        ...
        /// 获取在线用户列表
        /// </summary>
        public async Task<List<HbtOnlineUser>> GetOnlineUsersAsync()
        {
            var users = new List<HbtOnlineUser>();
            foreach (var userConnections in _userConnections.Values)
            {
                foreach (var connectionId in userConnections)
                {
                    if (_cache.TryGetValue<HbtOnlineUser>($"signalr:user:{connectionId}", out var user))
                    {
                        users.Add(user);
                    }
                }
