...
/// 命名辅助类
/// </summary>
public static class HbtNamingHelper
{
    /// <summary>
    /// 从表名中提取模块名
    /// </summary>
    ...
    /// <returns>模块名</returns>
    public static string ExtractModuleName(string tableName)
    {
        ...
        // 提取第一个_和第二个_之间的字符串
        string moduleName = tableName.Substring(firstUnderscoreIndex + 1, secondUnderscoreIndex - firstUnderscoreIndex - 1);

        // 转换为帕斯卡命名法
        if (!string.IsNullOrEmpty(moduleName))
        {
            return char.ToUpper(moduleName[0]) + moduleName.Substring(1).ToLower();
        }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.Domain.Entities.{moduleName}";
    }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.Application.Dtos.{moduleName}";
    }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.Application.Services.{moduleName}";
    }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.Domain.Repositories.{moduleName}";
    }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.Infrastructure.Repositories.{moduleName}";
    }
    ...
    {
        string moduleName = ExtractModuleName(tableName);
        return $"{baseNamespace}.WebApi.Controllers.{moduleName}";
    }
    ...
    public static string GetPermsPrefix(string tableName)
    {
        ...
        string moduleName = tableName.Substring(firstUnderscoreIndex + 1, secondUnderscoreIndex - firstUnderscoreIndex - 1).ToLower();
    ...
    {
        return ExtractModuleName(tableName);
    }
} 