...
/// 部门数据初始化类
/// </summary>
public class HbtDbSeedIdentityDept
{
    ...
    public HbtDbSeedIdentityDept(IHbtRepositoryFactory repositoryFactory, IHbtLogger logger)
    {
    ...
    public async Task<(int, int)> InitializeDeptAsync()
    {
        ...
        {
            var existingDept = await DeptRepository.GetFirstAsync(d => d.Id == dept.Id);
