...
public class HbtDbSeedGenTemplate
{
    ...
    /// 构造函数
    /// </summary>
    public HbtDbSeedGenTemplate(IHbtRepositoryFactory repositoryFactory, IHbtLogger logger)
    {
    ...
    public async Task<(int insertCount, int updateCount)> InitializeGenTemplateAsync()
    {
        ...
        {
            var existingTemplate = await GenTemplateRepository.GetFirstAsync(x => x.TemplateName == template.TemplateName);
