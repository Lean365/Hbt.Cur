...
namespace Lean.Hbt.Common.Helpers
{
    ...
    /// 邮件帮助类
    /// </summary>
    public static class HbtMailHelper
    {
        private static HbtMailOption? _mailOption;
        ...
        {
            var options = serviceProvider.GetService(typeof(IOptions<HbtMailOption>)) as IOptions<HbtMailOption>;
            _mailOption = options?.Value ?? throw new InvalidOperationException($"未找到 {HbtMailOption.Position} 配置节点");
        }
