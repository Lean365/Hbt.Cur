...
public class HbtTemplateEngine : IHbtTemplateEngine
{
    ...
    /// <exception cref="HbtTemplateException">模板渲染失败时抛出</exception>
    public async Task<string> RenderAsync(string templateName, HbtGeneratorModel model)
    {
        ...
        try
        {
            ...
            if (compiledTemplate.HasErrors)
            {
                ...
                throw new HbtTemplateException($"模板渲染失败,模板名称:{templateName},模板类别:{model.Table.TplCategory},表名:{model.Table.TableName}:\n{errors}");
            }
        ...
        catch (Exception ex)
        {
            ...
            throw new HbtTemplateException($"模板渲染失败,模板名称:{templateName},模板类别:{model.Table.TplCategory},表名:{model.Table.TableName}", ex);
        }
    }
        ...
        if (!_templateCache.TryGetValue(templateHash, out var compiledTemplate))
        {
            ...
            try
            {
                ...
                if (compiledTemplate.HasErrors)
                {
                    ...
                    throw new HbtTemplateException($"模板解析错误:\n{errors}");
                }
            ...
            catch (Exception ex)
            {
                ...
                throw new HbtTemplateException($"模板编译失败: {ex.Message}", ex);
            }
        }
        else
        {
    ...
    private TemplateContext CreateTemplateContext(HbtGeneratorModel model)
    {
        ...
        scriptObject["module_name"] = model.Table.ModuleName;
        scriptObject["author"] = model.Table.Author;
        ...
        tableObject["module_name"] = model.Table.ModuleName;
        ...
        if (model.Columns == null || !model.Columns.Any())
        {
            ...
            throw new HbtTemplateException($"表 {model.Table.TableName} 的列信息为空");
        }
        ...
        var columns = filteredColumns.Select(c => new
        {
            ...
            csharp_type = c.CsharpType,
            data_type = c.DbColumnType,
            csharp_field = c.CsharpField,
            csharp_length = c.CsharpLength,
            csharp_decimal_digits = c.CsharpDecimalDigits,
            is_increment = c.IsIncrement == 1,
            is_primary_key = c.IsPrimaryKey == 1,
            is_required = c.IsRequired == 1,
            is_insert = c.IsInsert == 1,
            is_edit = c.IsEdit == 1,
            is_list = c.IsList == 1,
 ...
 */";
    ...
    private string GetTemplateContent(string templateName, string tplCategory, HbtGeneratorModel model)
    {
        if (_config.TemplatePaths == null)
            throw new HbtTemplateException("模板路径配置为空");
        if (_config.TemplatePaths.WebHostEnvironment == null)
            throw new HbtTemplateException("WebHostEnvironment 未配置");
        ...
        if (tplType == "1")
        {
            ...
            if (template == null)
            {
                ...
                throw new HbtTemplateException($"未找到数据库模板: {templateName}, 类别: {tplCategory}");
            }
