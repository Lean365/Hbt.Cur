...
//===================================================================
// 项目名 : Lean.Hbt
// 文件名 : HbtNewsTopicParticipantService.cs
// 创建者 : Claude
// 创建时间: 2024-12-01
// 版本号 : V0.0.1
// 描述   : 新闻话题参与者服务实现
...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-12-01
    /// </remarks>
    public class HbtNewsTopicParticipantService : HbtBaseService, IHbtNewsTopicParticipantService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        public HbtNewsTopicParticipantService(
            IHbtLogger logger,
        ...
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNewsTopicParticipant, bool>> TopicParticipantQueryExpression(HbtNewsTopicParticipantQueryDto? query)
        {
            return Expressionable.Create<HbtNewsTopicParticipant>()
        ...
        public async Task<HbtPagedResult<HbtNewsTopicParticipantDto>> GetListAsync(HbtNewsTopicParticipantQueryDto? query)
        {
            query ??= new HbtNewsTopicParticipantQueryDto();
        ...
        public async Task<HbtNewsTopicParticipantDto> GetByIdAsync(long id)
        {
            var participant = await TopicParticipantRepository.GetByIdAsync(id);
            if (participant == null)
            {
                throw HbtException.NotFound($"新闻话题参与者ID {id} 不存在");
            }

            return participant.Adapt<HbtNewsTopicParticipantDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsTopicParticipantCreateDto input)
        {
            var participant = input.Adapt<HbtNewsTopicParticipant>();
            ...
            return result > 0 ? participant.Id : throw new HbtException("创建新闻话题参与者失败");
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsTopicParticipantUpdateDto input)
        {
            var existingParticipant = await TopicParticipantRepository.GetByIdAsync(input.ParticipantId);
            if (existingParticipant == null)
            {
                throw HbtException.NotFound($"新闻话题参与者ID {input.ParticipantId} 不存在");
            }
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> UpdateStatusAsync(HbtNewsTopicParticipantStatusDto input)
        {
        ...
        public async Task<HbtPagedResult<HbtNewsTopicParticipantDto>> GetUserJoinedTopicsAsync(long userId, int pageIndex = 1, int pageSize = 20)
        {
            ...
            return new HbtPagedResult<HbtNewsTopicParticipantDto>
            {
                ...
                PageIndex = pageIndex,
                PageSize = pageSize,
                Rows = participants.Rows.Adapt<List<HbtNewsTopicParticipantDto>>()
            };
        }
        ...
        /// <param name="pageSize">页大小</param>
        /// <returns>参与者分页列表</returns>
        public async Task<HbtPagedResult<HbtNewsTopicParticipantDto>> SearchParticipantsAsync(string keyword, int pageIndex = 1, int pageSize = 20)
        {
            var result = await TopicParticipantRepository.GetPagedListAsync(
        ...
        public async Task<List<HbtNewsTopicParticipantDto>> GetActiveUsersAsync(long topicId, int count = 10)
        {
            ...
            return participants.OrderByDescending(x => x.LastActiveTime)
                .Take(count)
                .Adapt<List<HbtNewsTopicParticipantDto>>();
        }
        ...
        public async Task<List<HbtNewsTopicParticipantDto>> GetTopContributorsAsync(long topicId, int count = 10)
        {
            var participants = await TopicParticipantRepository.GetListAsync(x => 
                x.TopicId == topicId && x.ParticipationStatus == 1 && x.IsDeleted == 0);
