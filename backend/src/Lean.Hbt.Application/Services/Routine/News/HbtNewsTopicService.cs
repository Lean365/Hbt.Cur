...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-01-01
    /// </remarks>
    public class HbtNewsTopicService : HbtBaseService, IHbtNewsTopicService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 获取新闻话题仓储
        /// </summary>
        private IHbtRepository<HbtNewsTopic> TopicRepository => _repositoryFactory.GetBusinessRepository<HbtNewsTopic>();

        /// <summary>
        /// 获取新闻话题关系仓储
        /// </summary>
        private IHbtRepository<HbtNewsTopicRelation> TopicRelationRepository => _repositoryFactory.GetBusinessRepository<HbtNewsTopicRelation>();

        /// <summary>
        /// 获取新闻话题参与者仓储
        /// </summary>
        private IHbtRepository<HbtNewsTopicParticipant> TopicParticipantRepository => _repositoryFactory.GetBusinessRepository<HbtNewsTopicParticipant>();
        ...
        private Expression<Func<HbtNewsTopic, bool>> TopicQueryExpression(HbtNewsTopicQueryDto query)
        {
            return Expressionable.Create<HbtNewsTopic>()
        ...
        public async Task<HbtPagedResult<HbtNewsTopicDto>> GetListAsync(HbtNewsTopicQueryDto? query)
        {
            query ??= new HbtNewsTopicQueryDto();
            ...
            return new HbtPagedResult<HbtNewsTopicDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = query.PageIndex,
                PageSize = query.PageSize,
                Rows = result.Rows.Adapt<List<HbtNewsTopicDto>>()
            };
        }
        ...
        public async Task<HbtNewsTopicDto> GetByIdAsync(long id)
        {
            var topic = await TopicRepository.GetByIdAsync(id);
            return topic == null ? throw new HbtException(L("Routine.NewsTopic.NotFound", id)) : topic.Adapt<HbtNewsTopicDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsTopicCreateDto input)
        {
            // 检查话题名称是否已存在
            var existingTopic = await TopicRepository.GetFirstAsync(x => x.TopicName == input.TopicName);
            if (existingTopic != null)
            {
                throw new HbtException(L("Routine.NewsTopic.NameExists", input.TopicName));
            }

            var topic = input.Adapt<HbtNewsTopic>();
            ...
            throw new HbtException(L("Routine.NewsTopic.CreateFailed"));
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsTopicUpdateDto input)
        {
            var topic = await TopicRepository.GetByIdAsync(input.TopicId)
                ?? throw new HbtException(L("Routine.NewsTopic.NotFound", input.TopicId));
        ...
        {
            var topic = await TopicRepository.GetByIdAsync(id)
                ?? throw new HbtException(L("Routine.NewsTopic.NotFound", id));
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> UpdateStatusAsync(HbtNewsTopicStatusDto input)
        {
            var topic = await TopicRepository.GetByIdAsync(input.TopicId)
                ?? throw new HbtException(L("Routine.NewsTopic.NotFound", input.TopicId));
        ...
        public async Task<List<HbtNewsTopicDto>> GetHotTopicsAsync(int count = 10)
        {
            var topics = await TopicRepository.GetListAsync(x => x.TopicIsHot == 1 && x.Status == 1);
            return topics.OrderByDescending(x => x.TopicParticipantCount).Take(count).Adapt<List<HbtNewsTopicDto>>();
        }
        ...
        public async Task<List<HbtNewsTopicDto>> GetRecommendedTopicsAsync(int count = 10)
        {
            var topics = await TopicRepository.GetListAsync(x => x.TopicIsRecommend == 1 && x.Status == 1);
            return topics.OrderByDescending(x => x.CreateTime).Take(count).Adapt<List<HbtNewsTopicDto>>();
        }
        ...
        public async Task<HbtPagedResult<HbtNewsDto>> GetNewsByTopicIdAsync(long topicId, int pageIndex = 1, int pageSize = 20)
        {
            ...
            return new HbtPagedResult<HbtNewsDto>
            {
                TotalNum = relations.TotalNum,
                PageIndex = pageIndex,
                PageSize = pageSize,
                Rows = newsList.Adapt<List<HbtNewsDto>>()
            };
        }
        ...
        public async Task<HbtPagedResult<HbtNewsTopicParticipantDto>> GetTopicParticipantsAsync(long topicId, int pageIndex = 1, int pageSize = 20)
        {
            ...
            return new HbtPagedResult<HbtNewsTopicParticipantDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = pageIndex,
                PageSize = pageSize,
                Rows = result.Rows.Adapt<List<HbtNewsTopicParticipantDto>>()
            };
        }
        ...
        public async Task<bool> JoinTopicAsync(long topicId, long userId, string userName, string? userAvatar = null)
        {
            ...
            if (topic == null)
            {
                throw new HbtException(L("Routine.NewsTopic.NotFound", topicId));
            }
            ...
            if (existingParticipant != null)
            {
                ...
                {
                    throw new HbtException(L("Routine.NewsTopic.AlreadyJoined"));
                }
                else
                {
                    // 重新参与
            ...
            // 新增参与者
            var participant = new HbtNewsTopicParticipant
            {
                TopicId = topicId,
                UserId = userId,
                UserName = userName,
        ...
        public async Task<bool> LeaveTopicAsync(long topicId, long userId)
        {
            ...
            if (participant == null)
            {
                throw new HbtException(L("Routine.NewsTopic.NotJoined"));
            }
        ...
        public async Task<HbtPagedResult<HbtNewsTopicDto>> GetUserJoinedTopicsAsync(long userId, int pageIndex = 1, int pageSize = 20)
        {
            ...
            return new HbtPagedResult<HbtNewsTopicDto>
            {
                ...
                PageIndex = pageIndex,
                PageSize = pageSize,
                Rows = topics.Adapt<List<HbtNewsTopicDto>>()
            };
        }
        ...
        /// <param name="pageSize">页大小</param>
        /// <returns>话题分页列表</returns>
        public async Task<HbtPagedResult<HbtNewsTopicDto>> SearchTopicsAsync(string keyword, int pageIndex = 1, int pageSize = 20)
        {
            var result = await TopicRepository.GetPagedListAsync(
