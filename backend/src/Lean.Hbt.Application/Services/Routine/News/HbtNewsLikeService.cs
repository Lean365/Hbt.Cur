...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-01-01
    /// </remarks>
    public class HbtNewsLikeService : HbtBaseService, IHbtNewsLikeService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 获取新闻点赞仓储
        /// </summary>
        private IHbtRepository<HbtNewsLike> LikeRepository => _repositoryFactory.GetBusinessRepository<HbtNewsLike>();
        ...
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNewsLike, bool>> LikeQueryExpression(HbtNewsLikeQueryDto query)
        {
            return Expressionable.Create<HbtNewsLike>()
        ...
        public async Task<HbtPagedResult<HbtNewsLikeDto>> GetListAsync(HbtNewsLikeQueryDto? query)
        {
            query ??= new HbtNewsLikeQueryDto();
            ...
            return new HbtPagedResult<HbtNewsLikeDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = query.PageIndex,
                PageSize = query.PageSize,
                Rows = result.Rows.Adapt<List<HbtNewsLikeDto>>()
            };
        }
        ...
        public async Task<HbtNewsLikeDto> GetByIdAsync(long id)
        {
            var like = await LikeRepository.GetByIdAsync(id);
            return like == null ? throw new HbtException(L("Routine.NewsLike.NotFound", id)) : like.Adapt<HbtNewsLikeDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsLikeCreateDto input)
        {
            // 验证新闻是否存在
            ...
            {
                throw new HbtException(L("Routine.NewsLike.AlreadyLiked"));
            }

            var like = input.Adapt<HbtNewsLike>();
            ...
            throw new HbtException(L("Routine.NewsLike.CreateFailed"));
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsLikeUpdateDto input)
        {
            var like = await LikeRepository.GetByIdAsync(input.LikeId)
                ?? throw new HbtException(L("Routine.NewsLike.NotFound", input.LikeId));
        ...
        {
            var like = await LikeRepository.GetByIdAsync(id)
                ?? throw new HbtException(L("Routine.NewsLike.NotFound", id));
        ...
        public async Task<bool> LikeNewsAsync(long newsId, long userId, string userName, string? userAvatar = null)
        {
            try
            {
                ...
                var like = new HbtNewsLikeCreateDto
                {
                    NewsId = newsId,
                    UserId = userId,
                    UserName = userName,
        ...
        public async Task<HbtPagedResult<HbtNewsLikeDto>> GetLikedUsersByNewsIdAsync(long newsId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsLikeQueryDto
            {
                NewsId = newsId,
                PageIndex = pageIndex,
                PageSize = pageSize
        ...
        public async Task<HbtPagedResult<HbtNewsLikeDto>> GetLikedNewsByUserIdAsync(long userId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsLikeQueryDto
            {
                UserId = userId,
