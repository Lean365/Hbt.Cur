...
//===================================================================
// 项目名 : Lean.Hbt
// 文件名 : HbtNewsTopicRelationService.cs
// 创建者 : Claude
// 创建时间: 2024-12-01
// 版本号 : V0.0.1
// 描述   : 新闻话题关系服务实现
...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-12-01
    /// </remarks>
    public class HbtNewsTopicRelationService : HbtBaseService, IHbtNewsTopicRelationService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        public HbtNewsTopicRelationService(
            IHbtLogger logger,
        ...
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNewsTopicRelation, bool>> TopicRelationQueryExpression(HbtNewsTopicRelationQueryDto? query)
        {
            return Expressionable.Create<HbtNewsTopicRelation>()
        ...
        public async Task<HbtPagedResult<HbtNewsTopicRelationDto>> GetListAsync(HbtNewsTopicRelationQueryDto? query)
        {
            query ??= new HbtNewsTopicRelationQueryDto();
            ...
            return new HbtPagedResult<HbtNewsTopicRelationDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = query.PageIndex,
                PageSize = query.PageSize,
                Rows = result.Rows.Adapt<List<HbtNewsTopicRelationDto>>()
            };
        }
        ...
        public async Task<HbtNewsTopicRelationDto> GetByIdAsync(long id)
        {
            var relation = await TopicRelationRepository.GetByIdAsync(id);
            if (relation == null)
            {
                throw HbtException.NotFound($"新闻话题关系ID {id} 不存在");
            }

            return relation.Adapt<HbtNewsTopicRelationDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsTopicRelationCreateDto input)
        {
            var relation = input.Adapt<HbtNewsTopicRelation>();
            ...
            return result > 0 ? relation.Id : throw new HbtException("创建新闻话题关系失败");
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsTopicRelationUpdateDto input)
        {
            var existingRelation = await TopicRelationRepository.GetByIdAsync(input.RelationId);
            if (existingRelation == null)
            {
                throw HbtException.NotFound($"新闻话题关系ID {input.RelationId} 不存在");
            }
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> UpdateStatusAsync(HbtNewsTopicRelationStatusDto input)
        {
        ...
        public async Task<bool> RelateNewsToTopicAsync(long newsId, long topicId, int relationType = 1, int relationWeight = 1, bool isAutoRelation = false)
        {
            ...
            // 创建新的关联
            var relation = new HbtNewsTopicRelation
            {
                NewsId = newsId,
                TopicId = topicId,
        ...
        public async Task<List<HbtNewsTopicRelationDto>> GetTopicsByNewsIdAsync(long newsId)
        {
            var relations = await TopicRelationRepository.GetListAsync(x => 
                x.NewsId == newsId && x.IsDeleted == 0);

            return relations.Adapt<List<HbtNewsTopicRelationDto>>();
        }
        ...
        public async Task<List<HbtNewsTopicRelationDto>> GetNewsByTopicIdAsync(long topicId)
        {
            var relations = await TopicRelationRepository.GetListAsync(x => 
        ...
        public async Task<List<HbtNewsTopicDto>> MatchTopicsByKeywordsAsync(string keywords, int count = 5)
        {
            ...
            return topics.Take(count).Adapt<List<HbtNewsTopicDto>>();
        }
    }
} 