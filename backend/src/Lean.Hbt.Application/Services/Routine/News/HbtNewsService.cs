...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-12-01
    /// </remarks>
    public class HbtNewsService : HbtBaseService, IHbtNewsService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNews, bool>> NewsQueryExpression(HbtNewsQueryDto query)
        {
            return Expressionable.Create<HbtNews>()
        ...
        public async Task<HbtPagedResult<HbtNewsDto>> GetListAsync(HbtNewsQueryDto query)
        {
            query ??= new HbtNewsQueryDto();
            ...
            return new HbtPagedResult<HbtNewsDto>
            {
                ...
                Rows = result.Rows.Adapt<List<HbtNewsDto>>()
            };
        }
        ...
        public async Task<HbtNewsDto> GetByIdAsync(long id)
        {
            var news = await NewsRepository.GetByIdAsync(id);
            if (news == null)
            {
                throw HbtException.NotFound($"新闻ID {id} 不存在");
            }

            return news.Adapt<HbtNewsDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsCreateDto input)
        {
            var news = input.Adapt<HbtNews>();
            ...
            return result > 0 ? news.Id : throw new HbtException("创建新闻失败");
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsUpdateDto input)
        {
            var existingNews = await NewsRepository.GetByIdAsync(input.NewsId);
            if (existingNews == null)
            {
                throw HbtException.NotFound($"新闻ID {input.NewsId} 不存在");
            }
        ...
        public Task<(string fileName, byte[] content)> ExportAsync(HbtNewsQueryDto query, string sheetName)
        {
            // TODO: 实现Excel导出逻辑
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> UpdateStatusAsync(HbtNewsStatusDto input)
        {
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> AuditAsync(HbtNewsAuditDto input)
        {
            var news = await NewsRepository.GetByIdAsync(input.NewsId);
            if (news == null)
            {
            ...
            news.NewsAuditRemark = input.NewsAuditRemark;
            news.NewsAuditorBy = _currentUser.UserName ?? "system";
            news.NewsAuditTime = DateTime.Now;
            news.UpdateBy = _currentUser.UserName ?? "system";
            news.UpdateTime = DateTime.Now;
        ...
        public async Task<List<HbtNewsDto>> GetHotNewsAsync(int count = 10)
        {
            var newsList = await NewsRepository.GetListAsync(
                x => x.NewsIsHot == 1 && x.Status == 1 && x.IsDeleted == 0
            );

            return newsList.Take(count).OrderByDescending(x => x.NewsReadCount).Adapt<List<HbtNewsDto>>();
        }
        ...
        public async Task<List<HbtNewsDto>> GetRecommendNewsAsync(int count = 10)
        {
            var newsList = await NewsRepository.GetListAsync(
                x => x.NewsIsRecommend == 1 && x.Status == 1 && x.IsDeleted == 0
            );

            return newsList.Take(count).OrderByDescending(x => x.NewsRecommendCount).Adapt<List<HbtNewsDto>>();
        }
        ...
        public async Task<List<HbtNewsDto>> GetTopNewsAsync(int count = 5)
        {
            var newsList = await NewsRepository.GetListAsync(
                x => x.NewsIsTop == 1 && x.Status == 1 && x.IsDeleted == 0
            );

            return newsList.Take(count).OrderByDescending(x => x.OrderNum).Adapt<List<HbtNewsDto>>();
        }
        ...
        public async Task<List<HbtNewsDto>> GetNewsByCategoryAsync(string category, int count = 20)
        {
            var newsList = await NewsRepository.GetListAsync(
                x => x.NewsCategory == category && x.Status == 1 && x.IsDeleted == 0
            );

            return newsList.Take(count).OrderByDescending(x => x.CreateTime).Adapt<List<HbtNewsDto>>();
        }
        ...
        /// <param name="count">获取数量</param>
        /// <returns>新闻列表</returns>
        public async Task<List<HbtNewsDto>> SearchNewsAsync(string keyword, int count = 20)
        {
            var newsList = await NewsRepository.GetListAsync(
