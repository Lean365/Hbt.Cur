...
//===================================================================
// 项目名 : Lean.Hbt
// 文件名 : HbtNewsCommentLikeService.cs
// 创建者 : Lean365
// 创建时间: 2024-01-01
// 版本号 : V0.0.1
// 描述    : 新闻评论点赞服务实现
...
using Lean.Hbt.Domain.Entities.Routine.News;
...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-01-01
    /// </remarks>
    public class HbtNewsCommentLikeService : HbtBaseService, IHbtNewsCommentLikeService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        public HbtNewsCommentLikeService(
        ...
        /// 获取新闻评论点赞仓储
        /// </summary>
        private IHbtRepository<HbtNewsCommentLike> CommentLikeRepository => _repositoryFactory.GetBusinessRepository<HbtNewsCommentLike>();
        ...
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNewsCommentLike, bool>> CommentLikeQueryExpression(HbtNewsCommentLikeQueryDto query)
        {
            return Expressionable.Create<HbtNewsCommentLike>()
        ...
        public async Task<HbtPagedResult<HbtNewsCommentLikeDto>> GetListAsync(HbtNewsCommentLikeQueryDto? query)
        {
            query ??= new HbtNewsCommentLikeQueryDto();
            ...
            return new HbtPagedResult<HbtNewsCommentLikeDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = query.PageIndex,
                PageSize = query.PageSize,
                Rows = result.Rows.Adapt<List<HbtNewsCommentLikeDto>>()
            };
        }
        ...
        public async Task<HbtNewsCommentLikeDto> GetByIdAsync(long id)
        {
            var like = await CommentLikeRepository.GetByIdAsync(id);
            return like == null ? throw new HbtException(L("Routine.NewsCommentLike.NotFound", id)) : like.Adapt<HbtNewsCommentLikeDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsCommentLikeCreateDto input)
        {
            // 验证评论是否存在
            ...
            {
                throw new HbtException(L("Routine.NewsCommentLike.AlreadyLiked"));
            }

            var like = input.Adapt<HbtNewsCommentLike>();
            ...
            throw new HbtException(L("Routine.NewsCommentLike.CreateFailed"));
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsCommentLikeUpdateDto input)
        {
            var like = await CommentLikeRepository.GetByIdAsync(input.CommentLikeId)
                ?? throw new HbtException(L("Routine.NewsCommentLike.NotFound", input.CommentLikeId));
        ...
        {
            var like = await CommentLikeRepository.GetByIdAsync(id)
                ?? throw new HbtException(L("Routine.NewsCommentLike.NotFound", id));
        ...
        public async Task<bool> LikeCommentAsync(long commentId, long userId, string userName, string? userAvatar = null)
        {
            try
            {
                ...
                var like = new HbtNewsCommentLikeCreateDto
                {
                    CommentId = commentId,
                    UserId = userId,
                    UserName = userName,
        ...
        public async Task<HbtPagedResult<HbtNewsCommentLikeDto>> GetLikedUsersByCommentIdAsync(long commentId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentLikeQueryDto
            {
                CommentId = commentId,
                PageIndex = pageIndex,
                PageSize = pageSize
        ...
        public async Task<HbtPagedResult<HbtNewsCommentLikeDto>> GetLikedCommentsByUserIdAsync(long userId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentLikeQueryDto
            {
                UserId = userId,
