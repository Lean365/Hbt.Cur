...
//===================================================================
// 项目名 : Lean.Hbt
// 文件名 : HbtNewsCommentAuditWorkflowService.cs
// 创建者 : Lean365
// 创建时间: 2024-01-01
// 版本号 : V0.0.1
// 描述    : 新闻评论审核工作流服务
...
namespace Lean.Hbt.Application.Services.Routine.News
{
    ...
    /// 创建时间: 2024-01-01
    /// </remarks>
    public class HbtNewsCommentAuditWorkflowService : HbtBaseService, IHbtNewsCommentAuditWorkflowService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 评论服务
        /// </summary>
        protected readonly IHbtNewsCommentService _commentService;
        ...
        public HbtNewsCommentAuditWorkflowService(
        ...
        public async Task<HbtApiResult<bool>> ProcessCommentAuditWorkflowAsync(long commentId)
        {
            try
            {
                ...
                if (comment == null)
                {
                    return new HbtApiResult<bool> { Code = 404, Msg = "评论不存在", Data = false };
                }

                // 检查评论状态
                if (comment.CommentStatus != 0) // 不是待审核状态
                {
                    return new HbtApiResult<bool> { Code = 400, Msg = "评论状态不正确", Data = false };
                }
                ...
                return new HbtApiResult<bool> { Code = 200, Msg = "审核流程处理成功", Data = true };
            }
            catch (Exception ex)
            {
        ...
        private async Task ProcessManualAuditAsync(HbtNewsComment comment)
        {
        ...
        /// </summary>
        /// <param name="comment">评论实体</param>
        private async Task ProcessAutoAuditAsync(HbtNewsComment comment)
        {
            ...
            comment.AuditorBy = "系统自动审核";
            comment.AuditTime = DateTime.Now;
            comment.AuditRemark = auditResult.Remark;
            comment.AuditType = 2;
            comment.UpdateBy = _currentUser.UserName ?? "system";
            comment.UpdateTime = DateTime.Now;
        ...
        /// <param name="comment">评论实体</param>
        /// <returns>审核结果</returns>
        private async Task<AutoAuditResult> PerformAutoAuditAsync(HbtNewsComment comment)
        {
            ...
            {
                if (HbtWordFilterHelper.ContainsSensitiveWord(comment.CommentContent))
                {
        ...
        /// </summary>
        /// <param name="comment">评论实体</param>
        private async Task SendAuditNotificationAsync(HbtNewsComment comment)
        {
            try
            {
        ...
        public async Task<AuditStatistics> GetAuditStatisticsAsync()
        {
            ...
            var todayApprovedCount = await CommentRepository.GetCountAsync(x => x.CommentStatus == 1 && x.AuditTime >= today);
            var todayRejectedCount = await CommentRepository.GetCountAsync(x => x.CommentStatus == 2 && x.AuditTime >= today);
        ...
        public async Task<List<AuditorWorkload>> GetAuditorWorkloadAsync(DateTime startDate, DateTime endDate)
        {
            var auditRecords = await CommentRepository.GetListAsync(x =>
                x.AuditTime >= startDate &&
                x.AuditTime <= endDate &&
                !string.IsNullOrEmpty(x.AuditorBy));

            var auditorGroups = auditRecords
                .GroupBy(x => x.AuditorBy)
