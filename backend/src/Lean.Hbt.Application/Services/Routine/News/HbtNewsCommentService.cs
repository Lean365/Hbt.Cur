...
namespace Lean.Hbt.Application.Services.Routine.News
{
    /// <summary>
    /// 新闻评论服务实现
    /// </summary>
    ...
    /// 创建时间: 2024-01-01
    /// </remarks>
    public class HbtNewsCommentService : HbtBaseService, IHbtNewsCommentService
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 评论配置选项
        /// </summary>
        protected readonly HbtCommentOptions _commentOptions;
        ...
        public HbtNewsCommentService(
            IHbtRepositoryFactory repositoryFactory,
            IOptions<HbtCommentOptions> commentOptions,
            IHbtLogger logger,
        ...
        /// 获取新闻评论仓储
        /// </summary>
        private IHbtRepository<HbtNewsComment> CommentRepository => _repositoryFactory.GetBusinessRepository<HbtNewsComment>();

        /// <summary>
        /// 获取新闻仓储
        /// </summary>
        private IHbtRepository<HbtNews> NewsRepository => _repositoryFactory.GetBusinessRepository<HbtNews>();

        /// <summary>
        /// 构建查询条件
        /// </summary>
        private Expression<Func<HbtNewsComment, bool>> CommentQueryExpression(HbtNewsCommentQueryDto query)
        {
            return Expressionable.Create<HbtNewsComment>()
                ...
                .AndIF(!string.IsNullOrEmpty(query.AuditorBy), x => x.AuditorBy!.Contains(query.AuditorBy!))
        ...
        public async Task<HbtPagedResult<HbtNewsCommentDto>> GetListAsync(HbtNewsCommentQueryDto? query)
        {
            query ??= new HbtNewsCommentQueryDto();
            ...
            return new HbtPagedResult<HbtNewsCommentDto>
            {
                TotalNum = result.TotalNum,
                PageIndex = query.PageIndex,
                PageSize = query.PageSize,
                Rows = result.Rows.Adapt<List<HbtNewsCommentDto>>()
            };
        }
        ...
        public async Task<HbtNewsCommentDto> GetByIdAsync(long id)
        {
            var comment = await CommentRepository.GetByIdAsync(id);
            return comment == null ? throw new HbtException(L("Routine.NewsComment.NotFound", id)) : comment.Adapt<HbtNewsCommentDto>();
        }
        ...
        public async Task<long> CreateAsync(HbtNewsCommentCreateDto input)
        {
            // 验证评论内容字数
            ...
            if (news == null)
            {
                throw new HbtException(L("Routine.News.NotFound", input.NewsId));
            }
            ...
            if (input.ParentCommentId > 0)
            {
                ...
                {
                    throw new HbtException(L("Routine.NewsComment.ParentNotFound", input.ParentCommentId));
                }
            }

            var comment = input.Adapt<HbtNewsComment>();
            ...
            throw new HbtException(L("Routine.NewsComment.CreateFailed"));
        }
        ...
        public async Task<bool> UpdateAsync(HbtNewsCommentUpdateDto input)
        {
            // 验证评论内容字数
            if (_commentOptions.EnableLengthLimit)
            {
            ...
            var comment = await CommentRepository.GetByIdAsync(input.CommentId)
                ?? throw new HbtException(L("Routine.NewsComment.NotFound", input.CommentId));
        ...
        {
            var comment = await CommentRepository.GetByIdAsync(id)
                ?? throw new HbtException(L("Routine.NewsComment.NotFound", id));
        ...
        public async Task<HbtPagedResult<HbtNewsCommentDto>> GetCommentsByNewsIdAsync(long newsId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentQueryDto
            {
                NewsId = newsId,
        ...
        public async Task<HbtPagedResult<HbtNewsCommentDto>> GetRepliesByCommentIdAsync(long commentId, int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentQueryDto
            {
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> AuditAsync(HbtNewsCommentAuditDto input)
        {
            var comment = await CommentRepository.GetByIdAsync(input.CommentId)
                ?? throw new HbtException(L("Routine.NewsComment.NotFound", input.CommentId));
            ...
            comment.AuditorBy = _currentUser.UserName ?? "system";
            comment.AuditTime = DateTime.Now;
            comment.AuditRemark = input.AuditRemark;
            ...
            if (result > 0)
            {
                _logger.Info($"审核评论成功，评论ID：{input.CommentId}，状态：{input.CommentStatus}，审核人：{comment.AuditorBy}");
                return true;
            }
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> BatchAuditAsync(HbtNewsCommentBatchAuditDto input)
        {
            ...
            {
                comment.CommentStatus = input.CommentStatus;
                comment.AuditorBy = currentUser;
                comment.AuditTime = currentTime;
                comment.AuditRemark = input.AuditRemark;
                comment.AuditType = input.AuditType;
        ...
        /// <returns>是否成功</returns>
        public async Task<bool> ApproveAsync(long commentId, string? auditRemark = null)
        {
            var auditDto = new HbtNewsCommentAuditDto
            {
                CommentId = commentId,
                CommentStatus = 1, // 已通过
        ...
        /// <param name="auditRemark">审核备注</param>
        /// <returns>是否成功</returns>
        public async Task<bool> RejectAsync(long commentId, string? auditRemark = null)
        {
            var auditDto = new HbtNewsCommentAuditDto
            {
                CommentId = commentId,
                CommentStatus = 2, // 已拒绝
                AuditRemark = auditRemark ?? "审核拒绝",
                AuditType = 1
            };
        ...
        public async Task<HbtPagedResult<HbtNewsCommentDto>> GetPendingAuditListAsync(int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentQueryDto
            {
                CommentStatus = 0, // 待审核
        ...
        public async Task<HbtPagedResult<HbtNewsCommentDto>> GetAuditedListAsync(int pageIndex = 1, int pageSize = 20)
        {
            var query = new HbtNewsCommentQueryDto
            {
                CommentStatus = 1, // 已通过
        ...
        private void ValidateCommentLength(string content)
        {
            ...
            {
                throw new HbtException(L("Routine.NewsComment.ContentEmpty"));
            }
            ...
            {
                throw new HbtException(L("Routine.NewsComment.ContentTooShort", _commentOptions.MinCommentLength));
            }

            if (contentLength > _commentOptions.MaxCommentLength)
            {
                throw new HbtException(L("Routine.NewsComment.ContentTooLong", _commentOptions.MaxCommentLength));
            }
        }
        ...
        private async Task ValidateCommentRateLimitAsync(long userId)
        {
            ...
            if (lastCommentTime != null)
            {
                ...
                {
                    throw new HbtException(L("Routine.NewsComment.RateLimitExceeded", _commentOptions.CommentRateLimit));
                }
            }
        }
        ...
        private async Task ValidateIpCommentLimitAsync()
        {
            ...
            {
                throw new HbtException(L("Routine.NewsComment.IpLimitExceeded", _commentOptions.DailyIpCommentLimit));
            }
        }
        ...
        private async Task ValidateUserCommentLimitAsync(long userId)
        {
            ...
            {
                throw new HbtException(L("Routine.NewsComment.UserLimitExceeded", _commentOptions.DailyUserCommentLimit));
            }
        }
        ...
        private async Task ValidateDuplicateCommentAsync(string content, long userId)
        {
            ...
            {
                throw new HbtException(L("Routine.NewsComment.DuplicateContent"));
            }
        }
        ...
        private async Task<string> FilterSensitiveWordsAsync(string content)
        {
            ...
            {
                // 检查是否包含敏感词
                if (HbtWordFilterHelper.ContainsSensitiveWord(content))
                {
                    // 获取敏感词列表
                    var sensitiveWords = HbtWordFilterHelper.GetSensitiveWords(content);
                    _logger.Warn($"检测到敏感词：{string.Join(", ", sensitiveWords)}");

                    // 过滤敏感词
                    var filteredContent = HbtWordFilterHelper.FilterSensitiveWord(content);
