    ...
    public class HbtExpressionEngine : IHbtExpressionEngine
    {
        ...
        private void RegisterBuiltInFunctions()
        {
            ...
            RegisterFunction("Sum", async args =>
                await Task.Run(() => args.Select(a => Convert.ToDouble(a)).Sum()));

            RegisterFunction("Average", async args =>
                await Task.Run(() => args.Select(a => Convert.ToDouble(a)).Average()));

            // 字符串函数
            RegisterFunction("Concat", async args =>
                await Task.Run(() => string.Concat(args.Select(a => a?.ToString()))));

            RegisterFunction("Contains", async args =>
                await Task.Run(() => args[0]?.ToString()?.Contains(args[1]?.ToString() ?? "") ?? false));
            ...
            RegisterFunction("ToNumber", async args =>
                await Task.Run(() => Convert.ToDouble(args[0])));

            RegisterFunction("ToString", async args =>
                await Task.Run(() => args[0]?.ToString() ?? ""));

            RegisterFunction("ToDate", async args =>
                await Task.Run(() => Convert.ToDateTime(args[0])));

            // 条件函数
            RegisterFunction("If", async args =>
                await Task.Run(() => Convert.ToBoolean(args[0]) ? args[1] : args[2]));

            RegisterFunction("IsNull", async args =>
                await Task.Run(() => args[0] == null));

            RegisterFunction("IsEmpty", async args =>
                await Task.Run(() => string.IsNullOrEmpty(args[0]?.ToString())));
        }
