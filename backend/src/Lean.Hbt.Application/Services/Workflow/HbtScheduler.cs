...
namespace Lean.Hbt.Application.Services.Workflow
{
    ...
    /// 工作流调度器
    /// </summary>
    public class HbtScheduler : HbtBaseService, IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                // 设置Job工厂
                _scheduler.JobFactory = new HbtJobFactory(_serviceProvider);
        ...
        private Type GetJobType(int taskType)
        {
            ...
            {
                1 => typeof(HbtTimeoutReminderJob), // 超时提醒
                2 => typeof(HbtAutoExecutionJob), // 自动执行
                3 => typeof(HbtTimedTriggerJob), // 定时触发
                4 => typeof(HbtDelayedExecutionJob), // 延迟执行
                5 => typeof(HbtPeriodicExecutionJob), // 周期执行
    ...
    /// 工作流Job工厂
    /// </summary>
    public class HbtJobFactory : IJobFactory
    {
        private readonly IServiceProvider _serviceProvider;
        ...
        public HbtJobFactory(IServiceProvider serviceProvider)
        {
            _serviceProvider = serviceProvider;
        }
    ...
    /// 工作流任务扫描Job
    /// </summary>
    public class HbtTaskScannerJob : IJob
    {
        private readonly IServiceProvider _serviceProvider;
        ...
        /// <param name="serviceProvider">服务提供者</param>
        /// <param name="logger">日志服务</param>
        public HbtTaskScannerJob(IServiceProvider serviceProvider, IHbtLogger logger)
        {
            _serviceProvider = serviceProvider;
            _logger = logger;
        }
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                var taskService = scope.ServiceProvider.GetRequiredService<IHbtScheduledTaskService>();
                var scheduler = scope.ServiceProvider.GetRequiredService<HbtScheduler>();
