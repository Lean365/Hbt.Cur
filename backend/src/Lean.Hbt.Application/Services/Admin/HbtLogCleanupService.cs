...
namespace Lean.Hbt.Application.Services.Admin
{
    ...
    public class HbtLogCleanupService : IHbtLogCleanupService
    {
        ...
        public async Task<LogCleanupOptions> GetConfigAsync()
        {
            ...
            options.AuditLogRetentionDays = GetConfigValue(configs, "log.cleanup.audit.retention", _defaultOptions.AuditLogRetentionDays);
        ...
        public async Task CleanupAsync()
        {
            ...
            // 清理审计日志
            await CleanupAuditLogs(now.AddDays(-options.AuditLogRetentionDays), batchSize);
        ...
        private async Task CleanupAuditLogs(DateTime beforeTime, int batchSize)
        {
            while (true)
            {
                var logs = await _auditLogRepository.GetListAsync(l => l.CreateTime < beforeTime);
                if (!logs.Any()) break;

                await _auditLogRepository.DeleteAsync(logs.Take(batchSize).ToList());
            }
        }
