...
namespace Lean.Hbt.Application.Services.Audit
{
    ...
    public class HbtExceptionLogService : IHbtExceptionLogService
    {
        ...
        public async Task<HbtExceptionLogDto> GetAsync(long logId)
        {
            ...
            if (log == null)
                throw new HbtBusinessException($"异常日志不存在: {logId}");
        ...
        public async Task<byte[]> ExportAsync(HbtExceptionLogQueryDto query)
        {
            // 1.构建查询条件
            ...
            return await HbtExcelHelper.ExportAsync(exportDtos, "异常日志数据");
        }
