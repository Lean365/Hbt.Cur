...
namespace Lean.Hbt.Application.Services.Audit
{
    ...
    public class HbtDbDiffLogService : IHbtDbDiffLogService
    {
        ...
        public async Task<HbtDbDiffLogDto> GetAsync(long logId)
        {
            ...
            if (log == null)
                throw new HbtBusinessException($"数据库差异日志不存在: {logId}");
        ...
        public async Task<byte[]> ExportAsync(HbtDbDiffLogQueryDto query)
        {
            // 1.构建查询条件
            ...
            return await HbtExcelHelper.ExportAsync(exportDtos, "数据库差异日志数据");
        }
