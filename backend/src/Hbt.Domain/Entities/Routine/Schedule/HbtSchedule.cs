...
public class HbtSchedule : HbtBaseEntity
{
    /// <summary>
    /// 日程标题
    /// </summary>
    ...
    /// </summary>
    [SugarColumn(ColumnName = "schedule_type", ColumnDescription = "日程类型（0：普通日程，1：重要日程，2：紧急日程）", IsNullable = false, DefaultValue = "0", ColumnDataType = "int", IsOnlyIgnoreUpdate = false, IsOnlyIgnoreInsert = false)]
    public int ScheduleType { get; set; }

    /// <summary>
    /// 日程状态（0：未开始，1：进行中，2：已完成，3：已取消）
    /// </summary>
    [SugarColumn(ColumnName = "status", ColumnDescription = "日程状态（0：未开始，1：进行中，2：已完成，3：已取消）", IsNullable = false, DefaultValue = "0", ColumnDataType = "int", IsOnlyIgnoreUpdate = false, IsOnlyIgnoreInsert = false)]
    ...
    /// </summary>
    [SugarColumn(ColumnName = "repeat_type", ColumnDescription = "重复类型（0：不重复，1：每天，2：每周，3：每月，4：每年）", IsNullable = false, DefaultValue = "0", ColumnDataType = "int", IsOnlyIgnoreUpdate = false, IsOnlyIgnoreInsert = false)]
