...
namespace Lean.Hbt.Domain.Entities.Logistics.Production
{
    ...
    /// 旧品管理主表实体（基于设变号码控制）
    /// </summary>
    [SugarTable("hbt_logistics_obsolete_item", "旧品管理主表")]
    ...
    public class HbtObsoleteItem : HbtBaseEntity
    {
        ...
        [Navigate(NavigateType.OneToMany, nameof(HbtObsoleteItemDetail.ObsoleteItemId))]
        public List<HbtObsoleteItemDetail> ObsoleteItemDetails { get; set; } = new();

        /// <summary>旧品处理记录列表</summary>
        [Navigate(NavigateType.OneToMany, nameof(HbtObsoleteItemProcess.ObsoleteItemId))]
        public List<HbtObsoleteItemProcess> ObsoleteItemProcesses { get; set; } = new();
    }
} 