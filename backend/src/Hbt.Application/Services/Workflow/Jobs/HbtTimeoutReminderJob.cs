...
namespace Hbt.Cur.Application.Services.Workflow.Jobs
{
    ...
    public class HbtTimeoutReminderJob : IJob
    {
        /// <summary>
        /// 仓储工厂
        /// </summary>
        ...
        /// 获取工作流操作记录仓储
        /// </summary>
        private IHbtRepository<HbtInstanceOper> InstanceOperRepository => _repositoryFactory.GetWorkflowRepository<HbtInstanceOper>();
        ...
        /// <returns>异步任务</returns>
        private async Task ProcessTimeoutInstanceAsync(HbtInstance instance)
        {
            try
            {
                // 检查是否真的超时
        ...
        /// <param name="instance">工作流实例</param>
        /// <returns>异步任务</returns>
        private async Task AutoStopWorkflowAsync(HbtInstance instance)
        {
            try
            {
                ...
                // 记录自动停止操作
                var stopRecord = new HbtInstanceOper
                {
                    ...
                    OperData = JsonSerializer.Serialize(new
                    {
        ...
        /// <param name="instance">工作流实例</param>
        /// <returns>异步任务</returns>
        private async Task CreateTimeoutReminderAsync(HbtInstance instance)
        {
            try
            {
                var reminderRecord = new HbtInstanceOper
                {
                    InstanceId = instance.Id,
                    NodeId = instance.CurrentNodeId ?? "",
                    NodeName = instance.CurrentNodeName ?? "",
        ...
        /// <param name="instance">工作流实例</param>
        /// <returns>异步任务</returns>
        private async Task SendTimeoutNotificationAsync(HbtInstance instance)
        {
            try
            {
