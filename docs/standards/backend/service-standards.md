...
- 按业务模块分子目录,例如:
  - `Lean.Hbt.Application.Services.Identity` (身份认证模块)
  - `Lean.Hbt.Application.Services.System` (系统管理模块)
  - `Lean.Hbt.Application.Services.Monitor` (监控模块)

## 2. 命名规范

- 接口必须以`IHbt`开头,以`Service`结尾
- 实现类必须以`Hbt`开头,以`Service`结尾
- 接口和实现类放在同一个命名空间下
