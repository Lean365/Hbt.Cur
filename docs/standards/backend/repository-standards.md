...
namespace Lean.Hbt.Domain.Repositories
{
    ...
    /// 创建时间: 2024-01-16
    /// </remarks>
    public interface IHbtRepository<TEntity> where TEntity : class
    {
        /// <summary>
        /// 获取SqlSugar客户端
        /// </summary>
...
2. 扩展用法
   ...
   - 对于特殊查询需求,在应用服务层通过注入`IHbtRepository<T>`来实现
   - 禁止创建单独的仓储实现类
...
5. 使用示例
   ...
   public class HbtUserService
   {
       ...
       // 使用通用仓储方法
       public async Task<HbtUser> GetUserAsync(long id)
       {
           return await _userRepository.GetByIdAsync(id);
       }

       // 特殊查询直接使用SqlSugarClient
       public async Task<List<HbtUser>> GetActiveUsersAsync()
       {
           return await _userRepository.SqlSugarClient.Queryable<HbtUser>()
